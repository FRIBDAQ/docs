...
build flagIf true (normal) glom builds events, if not,
                            then for diagnostic purposes, glom creates events
                            out of single fragments.

coincidence intervalIf the build flag is set, this is the number
...
timestamp policy.Determines how the timestamp for output events are determined
...
averageThe output time stamp is the average of all timestamps
...
| --- | --- | --- |
| Prev | Home | Next |
| CDataFormatItem (v12) | Up | CPhysicsEventItem  (v12) |
