...
`-p12000`Connect to  a TclServer that's listening on port
                            12000.  We'll talk later about how to set that up.

`-i5`Update the monitored variables at an interval
                            of 5 seconds.

`-nlocalhost`Indicates the program will connect to a Tcl server
...
`/scratch/fox/daq/master.git/main/cookbooks/epicspush/sbsreadout/channels.dat`The full path to the channels.dat
...
[[c665#epics.sbs.onstart]] `OnStart` is invoked when the
