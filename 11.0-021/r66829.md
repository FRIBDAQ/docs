...
Immediately performs an A32/D32 write of `data` to the address $`dev`+$`address`. The access is performed through the swig wrapped CVMUSB class provided as `ctlr`. The returns the status of the write operation (success = 0, failure otherwise).
...
Convenience function for gaining access of an internal bus. This is 
                      ...
                      The valid values for `code` are any bitwise OR of bus A (0x1), bus B (0x2), 
                      bus X (0x1000), and bus D (0x2000). The `ctlr` is a 
...
A convenience method for writing to the FPGA boot source register.
   Valid values for `source` are : 0x0, 0x1, 0x2, 0x3, and 0x10000. The meanings
   are as follows:
...
Loads firmware into SRAMA and then boots the device from SRAMA. The firmware is
    loaded from `filename`. The `ctlr` is a swig cvmusb::CVMUSB object.
...
| --- | --- | --- |
| Prev | Home | Next |
| XLM72ScalerGUI | Up | AXLM72Scaler |
