...
<a name="AEN12821"></a>## OPTIONS



`--formatted`If supplied the data this application sends to stdout will
...
`--source`=*ringurl*Determines the ring buffer from which data is taken.
...
`--sample`=*typeList*If specified, the `typeList` item types
...
`--exclude`=*typeList*Specifies the set of ring item types that will not be
...
`--accept`=*typeList*Specifies the set of ring bufer types that are accepted.
...
`--exitonend`If this is supplied, the ringselector
...
`--version`Prints out the version of the program and exists.

`--help`Prints out program usage information  and exits.

<a name="AEN12900"></a>## EXAMPLES
...
<a name="AEN12904"></a>**Example 1. Dumping state changes and sampled event data with od**
...
<a name="AEN12912"></a>**Example 2. Dumping all but packet types**
...
<a name="AEN12920"></a>**Example 3. Attaching SpecTcl**
