...
<a name="AEN71263"></a>## Name

marker -- Manage markers on a plotchart plot.

<a name="AEN71266"></a>## Synopsis
...
<a name="AEN71282"></a>## OPTIONS



`-plot` *plotchart-object*This option is mandatory at construction time and is the
                    Plotchart id of the plot on which the marker should
                    be displayed.  Once the marker is constructed, this
                    option is readonly.

`-size` *pixels*The size of the leg of the triangle in pixels.  This
                    defaults to 5 which is
                    usually fine.

`-direction` *direction-indicator*Indicates the orientation of the marker's 'point'.
...
`color` *color-spec*Specifies the outline color and, if the marker is drawn filled,
...
`-filled` *boolean*If the value of this option is true
