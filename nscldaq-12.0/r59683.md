...
<a name="AEN59701"></a>## COMMUNICATION PROTOCOL
...
0Operation was successful. In this case additional words on the
                    reply line may be data returned by the request.

-1When carrying out the request an error was reported from an
                    XIA API call. THe remainder of the line is a human readable
                    message that describes the error.

-1000A request was performed that required pixie16(s) to be
                    idle.  The remainder of the response line is an error
                    message that humans can understand.

-1001Parsing the request line failed. The remainder of the line
...
<a name="AEN59728"></a>### REQUEST DESCRIPTIONS



InventoryReturns an inventory of the modules configured into the
...
Boot *module-index*Reloads/Resets the firmware for the module
...
Readchanpar *module-index channel parameter-name*Reads a channel level parameter from module number
...
Readmodpar *module-index parameter-name*Reads a module level paramter from the module
...
Writechanpar *module-index channel parameter-name value*This operation is only legal for modules that are
...
Writemodpar *module-index parameter-name value*This operation is only legal for modules that are
                        not actively taking data.  If the plugin is being used
...
Saveparams *set-filename*This operation is only legal if
...
>Loadparams *set-filename*This request can only be
...
AdjustOffsets *module-number*This request performs an ADJUST_OFFSET run
...
pxisys.iniA file that describes the slot mapping in terms of the PCI
...
cfgPixie16.txtDescribes the module configuration of the crate. This
...
| --- | --- | --- |
| Prev | Home | Next |
| CInvalidArgumentException | Up | caen_nscldaq::Dig2Device |
