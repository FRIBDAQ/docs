...
<a name="AEN19878"></a>## Name

glom -- Glue Ordered Fragments Into Events

<a name="AEN19881"></a>## Synopsis

**$DAQBIN/glom `option....`
**

<a name="AEN19885"></a>## DESCRIPTION
...
`--dt`=*ticks*Specifies the coincidence window of timestamps in which
...
`-nobuild`If present, event building is not done.  Each physics fragment
                    is output as a separate ring item. This can be useful in debuggin.

`--timestamp-policy`=*policy*Selects the timestamp policy for built events.  This value determines
...
`--sourceid`=*id*Sets the source id of the output events.

`--maxfragments`=*frags*Defines the maximum number of fragments that can be built into
...
| --- | --- | --- |
| Prev | Home | Next |
| EVBRest | Up | Event builder client framework |
