...
# <a name="AEN77975"></a>multilogger

<a name="AEN77979"></a>## Name

multilogger -- Log data from multiple ring buffers

<a name="AEN77982"></a>## Synopsis
...
<a name="AEN77990"></a>## Using the multilogger package.
...
Record AlwaysIs a menu checkbutton that, when checked makes
                     ...
                     on the ReadoutGui is not checked.  If this is checked,
                     by default, multilogger will increment the run number after
                     each run when recording in the readout GUI was not enabled.
                     If, however the ReadoutCallouts.tcl e.g., sets the variable
                     `::multilogger::suppressIncrement` to 
                     a non-zero value, this is inhibited.   This can be useful
                     to suppress run number flickering when a detector system
                     such as Gretina is in charge of setting the run number.

After each menu item's dialogs are approved, the state of
              the multilogger subsystem is written to file.
              (See FILES) below.

<a name="AEN78028"></a>## FILES
