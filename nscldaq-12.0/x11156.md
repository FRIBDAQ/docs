...
[[`CRingItem`|r26274]]Base class for all the ring data item format classes.
...
[[`CRingStateChangeItem`|r27261]]Represents a state change item.  Given a reference to
...
[[`CRingScalerItem`|r26764]]Represents a state change item.  Given a reference to a
...
[[`CRingTextItem`|r27703]]Encapsulates a text list item.  As with all the above
                        classes, a constructor exists that converts a
                        `CRingItem` to a
                        `CRingTextItem` or throws a
                        `std::bad_cast` if that's not legal.

[[`CGlomParameters`|r29290]]Encapsulates the glom parameter ring items.  This ring
                        item describes how the glom component of the event builder
                        pipeline has been started.

[[`CPhysicsEventItem`|r28065]]Encapsulates the data taken in response to a physics
                        trigger.

[[`CRingFragmentItem`|r28623]]Encapsulates items produced in the ordering phase of
                        the event builder pipeline.  Items of this type are
                        believed to have other ring items as their payloads.

[[`CRingPhysicsEventCountItem`|r28261]]Encapsulates an item that gives information about the
...
[[`CUnknownFragment`|r28938]]Encapsulates an event fragment produced by the event
...
Reference information on these classes is availabe at:
                ...
                [[*CFileDataSource*|r32538]],
                [[*CDataSourceFactory"*|r32605]],
                and
