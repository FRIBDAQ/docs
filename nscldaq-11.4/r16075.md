...
<a name="AEN16127"></a>## METHODS



**define *callback-name***Defines a callback.   Clients can only register callback
...
**register *callback-name script***Registers a `script` for the
                            callback `callback-name`.

If `script` is empty, the
                            callback is assumed to have no script.

**get *callback-name***Returns the script that is registered on the
                            `callback-name`.  If there is no
                            script registered, an empty string is returned.

**invoke *callback-name substitutions values***Invokes the callback registered on `callback-name`.
...
| --- | --- | --- |
| Prev | Home | Next |
| EVB::BarrierStats::Summary | Up | EVB::connectionList |
