...
<a name="AEN77442"></a>## Name

v1495sc -- CAEN V1495 with Scaler firmware

<a name="AEN77445"></a>## Synopsis
...
<a name="AEN77467"></a>## OPTIONS



`-base`
*base-addresss*Allows you to override the base address set at creation
			 time.  The base address must be the address of a
			 V1495sc module.  Defaults to 0

`-exttrig ` enabled | disabledEnables the external trigger to latch scaler
			 values into the multi-event memory of the module.
			 The external trigger is the module's G0 input.
			 By default this is disabled.

`-inttrig` *enabled | disabled*Enables the internal trigger to latch values in to the
...
`-vmetrigger` *enabled |disabled*Enables or disables the software trigger.  This is used
...
`-inputlevel` *nim | ttl*Sets the input signalling levels expected by the
			 G0, and G1 inputs.  The default value is nim

`-autoreset` *on | off*If on (the default), the scalers are
			  atomically reset after a read.  This is suitable for use
			  in creating incremental scaler items.

`-g1_mode` *inhibit | reset | test*Sets the use of the G1 input.  If inhibit
...
`-dwelltime` *micro-seconds*When the internal trigger is enabled, this sets the
			 number of microseconds between internal triggers.
			 The default value is 0x2000000, an
