...
<a name="AEN34518"></a>## Public member functions
...
<a name="AEN34541"></a>` double getRevision(void);const`

Returns the firmware revision from the Id/firmware revision register.

<a name="AEN34552"></a>` void   LightOn(void);const`

Turns on the user LED.

<a name="AEN34563"></a>` void   LightOff(void);const`

Turns the user led off.

<a name="AEN34574"></a>` unsigned int getCsr(void);const`

Returns the contents of the control status register.

<a name="AEN34585"></a>` unsigned long getAcqMode(void);const`

Returns the contents of the acquisition mode register.

<a name="AEN34596"></a>` void   Reset(void);const`

Does a software reset on the module.

<a name="AEN34607"></a>` void   setOperatingMode(OperatingMode mode);const`

Set the modue operating mode.  See "Types and public data" below
                        for a description of the OperatingMode data type.

<a name="AEN34621"></a>` OperatingMode  getOperatingMode(void);const`

Returns the module operating mode.  See "Types and public data"
                        for a description of the OperatingMode data type.

<a name="AEN34633"></a>` void   setInputMode(InputMode mode);const`
...
<a name="AEN34647"></a>` InputMode getInputMode(void);const`
...
<a name="AEN34659"></a>` void setLatchSource(LNESource mode);const`
...
<a name="AEN34753"></a>` void DisableReferencePulser(void);const`

Disables the reference pulser.  See `EnableReferencePulser`
                            above for more information about the reference pulser.

<a name="AEN34765"></a>` bool isReferencePulserEnabled(void);const`
...
<a name="AEN34836"></a>` void> EnableClearOnLatch(void);const`

Enables the scalers to clear counters on the "Load next event"
                            signal.

<a name="AEN34847"></a>` unsigned long ReadChannel(unsigned int num);const`
...
<a name="AEN34860"></a>` void ReadAllChannels(unsigned long* buffer);const`
...
<a name="AEN34874"></a>` void Latch(void);const`
...
<a name="AEN34887"></a>` unsigned long ReadLatchedChannel(unsigned int num);const`
...
<a name="AEN34900"></a>` void ReadAllLatchedChannels(unsigned long* buffer);`
...
<a name="AEN34913"></a>` void LatchAndRead(unsigned long* buffer);const`

Peforms a load next event and reads all latch registers into the
                                `buffer`.

<a name="AEN34925"></a>## Types and public data

The CSIS3820.h defines several enumerated data types that
                are passed to the various setup functions of the module.

<a name="AEN34929"></a>### CSIS3820::OperatingMode

This enumerated type defines the module operating modes:



CSIS3820::LatchingScalerThe module will operate as a latching scaler.
                                    This is the only mode that is really supported at the NSCL.

CSIS3820::MultiChannelScalerIn this mode, the scaler takes data for fixed repetetive time
...
CSIS3820::HistogramingScalerIn this mode, the scaler performs much like the multiscaler mode
                                    described above except that the events saved are sums over all
                                    of the time periods.

<a name="AEN34949"></a>### CSIS3820::InputMode
...
CSIS3820::NoInputsThe inputs have no function (mode 0).

CSIS3820::InputLatchInhibitLatchInput 1 is an external load next event pulse.   Input 2 and 3
                                are external user inputs that are available in the control register.
                                Input 4 is an inhibit for the Load Next Event input.
                                (Mode 1).

CSIS3820::InputLatchInhibitAllAndLatch(Mode 2).  Input 1 is an external load next event input,
...
CSIS3820::InputLatchInhibitAll(Mode 3). Input 1 is the extneral latch next event. Inputs 2 and 3 are
...
CSIS3820::InhibitGroups(Mode 4).  Each input inhibits a group of 8 channels.
                                Input 1 inhibits channels 0-7, input 2 inhibits channels 8-15 etc.

CSIS3820::InputLNEHiscalMode 5: Input 1 is the load next event input while input two
                                starts the module's histogramming scaler data taking. All other
                                inputs have no function.

CSIS3820::InputLNEInhAllExtClearMode 6: Input 1 is the load next event input.  Input 2 inhibits counting
                                in all counters. input 3 clears all counters.

<a name="AEN34989"></a>### CSIS3820::LNESource
...
CSIS3820::LatchVMEonlyNext event an only be latched from the VME (via calls to functions
                                that latch the event such as `Latch`).

CSIS3820::LatchFPLatch only on front panel input.  This requires an input mode be
                                set that enables one of the front panel inputs as a Load next event
                                signal.

CSIS3820::Latch10MhzLatches based on the 10Mhz internal clock divided by the
                                latch preset register (not really supported)

CSIS3820::LatchChannelNLatch on the overflow of a channel selected
                                by the LNE channel select register.

CSIS3820::LatchPresetNLatch on counts in the selected scaler register divided
                                by the preset register value.

<a name="AEN35020"></a>## Exceptions
