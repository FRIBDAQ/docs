...
This chunk of the API provides access to waveform objects:



` void addWaveform(const CWaveform& wf);`Adds a copy of the waveform `wf` to the 
                    waveform dictionary.  If a waveform by this name already exists,
                    `CDuplicateSingleton` is thrown.  Note that the original
                    waveform parameter is not really usable.  First use `findWaveform`
                    to get a pointer to the copy of the waveform that this method created.

` void removeWaveform(const char* name);`Removes the named waveform from the waveform dictionary.  This should be done
                    with caution as it invalidates any pointers to the waveform that e.g. event processors
                    might be holding (this is why the **wavefom** command has no delete subcommand).
                    If the named waveform does not exist, `CNoSuchObjectException`
                    is thrown.

` size_t waveformCount();`Returns the number of known waveforms.

` CWaveFormDictionary::Dictionary_t::iterator waveformBegin();`Returns an iterator to the beginning of iteration in the waveform dictionary.
                    The iterator can be dereferenced to a an std::pair<std::string, CWaveform">
                    It can also be compared for equality with other iterators most often the one
                    returned from `waveformEnd`

` CWaveFormDictionary::Dictionary_t::iterator waveformEnd();`Returns an iterator off the end of the waveform dictionary.  Iteration
                    through the dictionary should end when the iterator is equal to this.
