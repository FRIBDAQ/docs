...
# <a name="AEN11330"></a>CTclAnalyzer

<a name="AEN11334"></a>## Name

CTclAnalyzer -- Analyzer integrated with Tcl supporting pipeline

<a name="AEN11337"></a>## Synopsis
...
BEGRUNBFInvokes `OnBegin`

ENDRUNBFInvokes
                                    `OnEnd`.

PAUSEBFInvokes `OnPause`

RESUMEBFInvokes `OnResume`
...
EventsRejectedThisRunSelects EventsRejectedThisRun. This
...
`PipelineElement`This is an element of the list that contains
...
`EventProcessingPipeline`The actual event processing pipeline.  This is actually a
                        `std::list<PipelineElement>`

`EventProcessorIterator`Represents an iterator into the event processing
                        pipeline container.  This is actually a
                        `EventProcessingPipeline::iterator`

`VariableArray`Defines a vector that contains TCL variables:
