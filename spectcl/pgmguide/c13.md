...
[[*The SpecTcl processing model.*|c73]]Which covers the processing model used by SpecTcl.  This will
...
[[*The SpecTcl event processing pipeline.*|c94]]The event processing pipeline is the set of objects that
...
[[*The SpecTcl event sink pipeline.*|c1539]]The event sink pipeline takes over where the event
...
[[*libTcl++ and adding commands to SpecTcl*|c1622]]SpecTcl uses the Tcl scripting language as its base command
...
[[*Extending SpecTcl spectrum file formats*|c1836]]The SpecTcl **sread** and **swrite**
...
[[*Extending SpecTcl's filter file formats*|c2080]]The format of files written by SpecTcl filters is extensible.
                    In this chapter we'll show how to extend the set of formats
                    available.

[[*Extending the set of SpecTcl fit types.*|c2382]]The SpecTcl fitting subsystem allows you to add additional
                    fit types.  In this chapter we'll show how to add a new
                    fit-type to SpecTcl.

[[*Interfacing SpecTcl with data from other data acquisition systems.*|c2824]]SpecTcl is data acquisition system neutral.  This chapter
...
[[*Dynamic processing pipelines*|c4359]]Describes new features for SpecTcl 5.1 which allow you to
                    dynamically alter the event processing pipeline.

[[*Capturing waveform data for display*|c4450]]Describes new features for SpecTcl 7.0-003 and later to support
