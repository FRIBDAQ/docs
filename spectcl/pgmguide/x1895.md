...
`create1DSpectrum`Creates and returns a pointer to
                        a uniquely named 1-d spectrum using a dummy
                        parameter.

`create2DSpectrum`Creates and returns a pointer to a uniquely named
                        2-d spectrum using a dummy parameter on both axes.

`dummyParameter`If a dummy parameter named
...
`uniqueName`Finds a spectrum name not yet in use for names like
...
[[x1895#spcfmt.read.scanlines]] `scanlines` will hold all of the spectrum
...
[[x1895#spcfmt.read.csvline]] `csvline` will hold the value of one scanline
...
| --- | --- | --- |
| Prev | Home | Next |
| Adding a file format to SpecTcl | Up | Extending SpecTcl's filter file formats |
