...
# <a name="AEN1850"></a>7.1. SpecTcl's spectrum file format infrastructure
...
`CSpectrumFormatter`Defined in SpectrumFormatter.h.
                        The base class for all formatters.  Each concrete
                        formatter must implement a `Read`
                        and `Write` method.

`CSpectrumFormatterFactory`Is a class that has only static methods (alternative
...
| --- | --- | --- |
| Prev | Home | Next |
| libTcl++ and adding commands to SpecTcl |  | Adding a file format to SpecTcl |
