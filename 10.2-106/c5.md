...
`create`Makes a new ring buffer.  We will see that in the
                        ringbuffer data acquisition system, each ring buffer
                        has a name.

`destroy`Destroys an existing ring buffer.  Note that the actual
                        destruction will occur when the last client has detached
                        from the shared memory region that holds the ring buffer.

`Connect`Connect to an existing ring buffer.

`put`Insert data into the ring buffer.

`get`Retrieve data from the ring buffer.

`waitUntil`Blocks until the ring buffer satisfies some
                        condition or  *predicate*.
                        This can be used to implement the blocking needed
                        for flow control for both the producer and the
                        consumer.

`peek`Exactly like `get` however
...
| --- | --- | --- |
| Prev | Home | Next |
| introduction | Up | Overview of ring buffer utilities |
