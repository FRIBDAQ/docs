...
The data format is described in detail in the [readout](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/readout.md) section. However, the interpretation of the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) time is unique to each module. The 3rd data word in the event header contains the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) time in its most significant 16 bits. The bits have the following meaning:
...
The table above shows that the timestamp in the module has a factor of two difference between the frequencies it timestamps with and it digitized with. This is really just a difference between the frequencies of the FPGA and ADC. This factor of two causes some interesting features in the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) time that the module outputs. The [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) time provides the correction for the fractional offset from the most recent sample point and also which sample point to adjust from (there are two samples per each tick on the timestamp clock). The meanings of the bits in the third 32-bit header word are as follow:
...
To compute the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) correction to the timestamp from this, one would do the following computation:
...
Like the 250 MSPS modules, there is a discepancy between the timestamp and ADC frequencies. The [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) time therefore also accounts for this detail. The third 32-bit header word emitted by the Pixie-500 module has the following bit meanings:
