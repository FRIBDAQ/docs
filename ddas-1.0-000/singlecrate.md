...
- [Running Readout from the ReadoutGUI](#ddas_ss1_readoutgui)- [Setting up the data sources](#ddas_ss1_dsourcesetup)
...
- Adding digitizer cards to the configuration shown is simply a change to the configuration files of the readout program (see e.g. [readout](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/dir_936f9f2cb541c58938765a02eba41f87.md) ) and below.
...
These files are described in the documentation of [readout](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/dir_936f9f2cb541c58938765a02eba41f87.md) . This section will summarize the files needed for nscope. By convention a directory subtree is used to hold the configuration files for each crate. The tree looks like this:
...
The directory `$DDAS_SHARE/readout/crate_1` has sample configuration files.
...
![nscope-startup.jpg](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/nscope-startup.jpg)
nscope start screen
...
Note that the nscope program controls many more parameters of the DPP algorithm. See [nscope](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/dir_0ed63cb803befae7f7665a2489cc5124.md) for more information about what the nscope program can do.
...
![ss1crate_jumpers.png](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/ss1crate_jumpers.png)
...
- Set the FD Thres. (ADC) to be the LED threshold required to trigger the channel. I'm going to use 1000. In general this signal should be below the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) fraction of the smallest waveform you will take, because the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) zero crossing must occur after the LED is satisfied.


Apply the settings. Finally enable the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) by selecting `UV_Setup->CSRA` and setting the `CF` bit in all of the channels for which you are going to use the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md).
...
For full documentation of the Readout program, see [readout](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/dir_936f9f2cb541c58938765a02eba41f87.md)
...
SourceID:     0
Barrier Type: 0
000c 0000 0064 0c0c 4020 0008 eda3 001f
0000 1b2f 2970 0000
-----------------------------------------------------------
Event 24 bytes long
Body Header:
...
SourceID:     0
Barrier Type: 0
000c 0000 0064 0c0c 4020 0008 a96a 0020
0000 1bff 2965 0000
-----------------------------------------------------------
Event 24 bytes long
Body Header:
...
SourceID:     0
Barrier Type: 0
000c 0000 0064 0c0c 4020 0008 652e 0021
0000 1ba2 2940 0000
-----------------------------------------------------------
Event 24 bytes long
Body Header:
...
The following longword is the low order 32 bits of the 48-bit event timestamp. Converted to decimal, this becomes 2,236,649. The actual timestamp placed in the event is in 1ns units. Since our digitizer is 100Mhz (10ns/sample), this is correct.


The next word is divided into two 16-bit fields, The low order 16 bits (0) are the high order 16 bits of the event timestamp. The high order 16 bits (0x1bdc = 7132) is the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) interpolation value. This is the fraction of a sample time multiplied by 65536. Therefore the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) zero crossing was interpolated at about .11 samples after the trigger time giving the high precision timestamp of 22366491.1 ns.


- Note
  The interpretation of the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.0-000/classCFD.md) bits above pertain specifically to the 100 MSPS Pixie-16 digitizers at the NSCL. The meanings of the bits changes slightly when working with the 250 MSPS and 500 MSPS digitizers.

# Running Readout from the ReadoutGUI


To run the readout GUI you must first have set up your account to allow ssh to login via rsa identity keys. If you have not done this yet, see e.g. [http://www.linuxproblem.org/art_9.html](http://www.linuxproblem.org/art_9.html)


- Note that this recipe assumes your current working directory is your home directory.
- Since NSCL systems share your home directories, in the step that reads:
...
EVBC::registerRingSource tcp://spdaq06/0400x_ddas_1 \
      $::env(DDAS_LIB)/libddastimestampextractor.so 0 {Crate\ 1\ data} (4)
...
4. EVBC::registerRingSource adds a ring data source to be started at the beginning of a run. This is a feeder from a ring buffer to an event builder that runs in the localhost. The parameters are, in order, the source ringbuffer URL, the timestamp extraction library the source id (must be the same as the –sourceid parameter for the Readout), and a comment that will be displayed in the event builder for this source. Spaces in the comment must be escaped as shown..
