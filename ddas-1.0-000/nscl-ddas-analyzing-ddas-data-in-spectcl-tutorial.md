...
Now that we have laid out the structure of the TreeParameters we will turn our attention to assigning data to them. Normally, users would be required to parse raw data in their SpecTcl and then assign values to the tree parameters. In the DDAS support, we parse the raw data for you. We will name the header file for our mapper [[MyParameterMapper_8h_source]]. It has the following contents:
...
The final work that needs to be done is to modify the Makefile. Add the [[structMyParameters]] and [[classMyParameterMapper]] class to the build by adding MyParameters.o and MyParameterMapper.o to the OBJECTS variable. It should look like this when you are done:


OBJECTS=MySpecTclApp.o [[structMyParameters]].o [[classMyParameterMapper]].o

 fragment Next the compiler needs to be told where to find the [[DDASBuiltUnpacker_8h_source]] file and that we need c++11 mode. That is done by adding to the USERCXXFLAGS so it looks like this:
