...
The key point is that the pipeline processes raw events from some file (e.g. event data acquired at the FRIB) to a simple self-describing data set that Rustogramr can understand without any further processing.  In [[./chap2_2]], we'll describe the format of the data expected by Rustogramer so that, if you want, you can use whatever other method you want to prepare your data.
...
We're going to ignore that.  Workers that decode raw event data are derived from the `frib::analysis::CMPIRawToParametersWorker`class. Our worker has to construct in a way that it can bind some of its data to the parameters defined in the [[chap2_1#preparing-the-parameter-definition-file]].  We then must override `unpackData` which actually unpacks a raw event into the parameters.
...
3. The `CTreeParameterArray` class represents a binding between an array like object (implements indexing) of real number like objects and names.  We'll bind this to the names of the paramters we made in our
   [[chap2_1#preparing-the-parameter-definition-file]]
...
10. The worker, method is called in each worker process. We instantiate the custom worker that we wrote in the [[chap2_1#the-worker-class]] section above.
...
[[chapter_3#using-the-port-manager-to-assign-server-port-values]] below for a description of this process and the prerequisites it requires.
...
[[chapter_3#running-rustogramer-with-hard-coded-server-port-values]] above.
...
(see [[chap4_1#the-spectrum-listing]]).
...
- [[chap4_1#1-d-spectrum]]
- [[chap4_1#2-d-spectrum]]
- [[chap4_1#summary-spectrum]]
- [[chap4_1#gamma-1-d-spectrum]]
- [[chap4_1#gamma-2-d-spectrum]]
- [[chap4_1#particle-gamma-spectrum]]
- [[chap4_1#2-d-sum-spectrum]]
- [[chap4_1#projection-spectra]]
- [[chap4_1#strip-charts]]
- [[chap4_1#bit-mask-parameters]]
- [[chap4_1#gamma-summary-spectra]]
...
[[chap4_1#summary-spectrum]], however:  There are two editable parameter lists. Having selected a parameter you can put it in either the X or Y parameters list depending on which of the arrow buttons you click.
...
The editor is identical, however to the [[chap4_1#particle-gamma-spectrum]]:
...
- [[chap4_4#slice-conditions]]
- [[chap4_4#contour-condition]]
- [[chap4_4#and-and-or-conditions]]
- [[chap4_4#not-conditions]]
- [[chap4_4#band-conditions]]
- [[chap4_4#gamma-bands-and-contours]]
- [[chap4_4#gamma-slice-conditions]]
- [[chap4_4#bit-mask-conditions]]
- [[chap4_4#true-and-false-conditions]]
...
- [[chap4_bindsets#new-bindset]] - makes a new bindset.
- [[chap4_bindsets#edit-bindset]] - edits the selected bindeset.
...
- [[chap4_bindsets#save-selected]] - Save the currently bound spectra to the selected bindset
- [[chap4_bindsets#save-new]] - Save the currently bound spectra to a new bindset.
...
the [[chap4_bindsets#new-bindset]] section.  However, the editor is first populated with the
...
- [[chap4_5#file-save]]
- [[chap4_5#file-save-treevariables]] (SpecTcl only)
- [[chap4_5#file-save-spectrum-contents]]
- [[chap4_5#file-load]]
- [[chap4_5#file-read-spectrum-contents]]
- [[chap4_5#file-source-tcl-script]] (SpecTcl only)
- [[chap4_5#file-exit]]
- [[chap4_5#file-stop-histogramer]] (Rustogramer only)


## [File->Save...](#file-save)


This menu item saves the cofiguration in an SQLite3 database file.  The schema for these files closely follows the SpecTcl database storage schema and is described in [[./chap7_6]].  You will be prompted for a filename and the following will be saved:
...
SpecTcl only can save tree variable definitions to an Sqlite3 data base file.  The schema for that file is the same as for the file saved by the [[chap4_5#file-save]] operation, however only the tables germane to tree parameter definitions are populated.
...
- [[./chap7_7]] (click the link for format details).
...
Loads a configuration file from an [[./chap7_6]] into the server.   It is the GUI that interprets the database file contents.   After being prompted for the name a file, parameter definitions will be loaded from that file. You'll be prompted for what to do with duplicate spectrum definitions:
...
- File format; as described in [[chap4_5#file-save-spectrum-contents]], the format of the file that is being read.
...
After prompting if you are sure, exits the GUI.  Note that in general the server continues to run and you can connect to it again later with a new GUI instance.  For Rustogramer, see, however [[chap4_5#file-stop-histogramer]] below.
...
- [[chap4_6#data-source-online]]  (SpecTcl only) Select an NSCLDAQ helper (e.g. rinselector) and take data from an online system.
- [[chap4_6#data-source-file]]  Take data from a file.  Note that as of SpecTcl version 5.13-002,  SpecTcl can analyze data from a parameter file prepared for Rustogramer as well as from raw event files.
- [[chap4_6#data-source-pipe]] (SpecTcl only) Read data from an arbitrary helper program.
- [[chap4_6#data-source-cluster-file]] (SpecTcl 5.14 and later only) Use a file to drive analysis from several event files.
- [[chap4_6#data-source-filter-file]] (SpecTcl only) take data from a filter file.
- [[chap4_6#data-source-detach]] Stop analyzing data from the current data source.
- [[chap4_6#data-source-abort-cluster-file]] (SpecTcl only) abort an in progress cluster file.
...
- The ability to create filters via a [[chap4_filters#filter-filter-wizard]].
- The ability to [[chap4_filters#filter-enabledisable-filters]]; enabled filters write data.
- Another menu entry to [[chap4_filters#filter-read-filter-files]] for analyais.
...
- [[chap4_filters#naming-the-filter]]
- [[chap4_filters#selecting-the-filter-gate]] to determine which events are written to the  filter file.
- [[chap4_filters#selecting-the-parameters-to-output]] to write for each event that makes the gate true.
- [[chap4_filters#choosing-the-output-file]] to which filter data will be written and whether the filter should be created
  enabled.
...
- [[chap4_7#save-spectrum-contents]]
- [[chap4_7#read-spectrum-file]]
- [[chap4_7#clear-all]]
- [[chap4_7#create]]
- [[chap4_7#delete]]
- [[chap4_7#apply-gate]]
...
- [[chap4_8#create-gates]] one or more gates
- [[chap4_8#apply-gate]] to one or more spectra.
- [[chap4_8#delete-gate]] a gate.
...
See the various gate editor descriptions in the [[./chap4_4]]


## [Apply gate](#apply-gate-1)


This functionality is the same as that in [[./chap4_7#apply-gate]]
...
- [[./chap6_1]] which you can use to create/port GUIs using Tcl/Tk
- [[./chap6_2]]  which also includes a sample fully featured [[./chapter_4]]


To be clear, these are client side APIs.  If you are interested in contributing a binding for another language, [[./chap7_2]]
...
[[chap6_1#using-the-low-level-client]].  To use it you must:
...
1. The `SpecTclRestCommand::initialize` command sets the connection parameters for the pacakge.  The parameters correspond in turn to the `-host` and `-port` option values to the constructor for the `SZpecTclRestClient` described in the [[chap6_1#using-the-low-level-client]].
...
1. We've extracted the program fragment that shows how to extend the import path [[chap6_2#extending-the-import-path-and-importing-the-module]] into the function `extedn_path` here we call that in the main program.
...
- The client, when making requests of SpecTcl must be able to accept data with `Content-Encoding: deflate` as this is used in the resonses to the [[./chap7_2_spectrum#spectclspectrumcontents]].
...
- [[chap7_2_parameter#spectclparameterlist]] - lists the parameters and their properties.
- [[chap7_2_parameter#spectclparameteredit]] - Modifies the metadata associated with a parameter.
- [[chap7_2_parameter#spectclparameterpromote]] - Promote a raw parameter to a tree parameter.
- [[chap7_2_parameter#spectclparametercreate]] - Create a new parameter (Rustogramer only)
- [[chap7_2_parameter#spectclparameterlistnew]] - lists parameters that have modified metadata.
- [[chap7_2_parameter#spectclparametercheck]] - Checks the modified state of parameters.
- [[chap7_2_parameter#spectclparameteruncheck]] - Turns off the modified state of parameters.
- [[chap7_2_parameter#spectclparameterversion]] - Provides version information about the capabilities of the parameter system (earlier versions of the SpecTcl `treeparameter` did not support the `-create` operation).
...
- Rustogramer - this is equivalent to the [[chap7_2_parameter#spectclparameteredit]] operation.
...
- [[chap7_2_rawparameter#spectclrawparameternew]] (Rustogramer maps this to [[./chap7_2_parameter#spectclparametercreate]]).  SpecTcl creates a new raw parameter.
- [[chap7_2_rawparameter#spectclrawparameterdelete]] (Rustogramer implements this).  This deletes an existing (raw) parameter.
- [[chap7_2_rawparameter#spectclrawparameterlist]] (Rustogramer maps this to [[./chap7_2_parameter#spectclparameterlist]])
...
- [[chap7_2_gates#spectclgatelist]] - lists defined conditions.
- [[chap7_2_gates#spectclgatedelete]] - Delets a condition
- [[chap7_2_gates#spectclgateedit]] - Create or modify a condition.
...
- [[chap7_2_spectrum#spectclspectrumlist]]  List spectra.
- [[chap7_2_spectrum#spectclspectrumdelete]] Delete an existing spectrum.
- [[chap7_2_spectrum#spectclspectrumcreate]] Create a new spectrum.
- [[chap7_2_spectrum#spectclspectrumcontents]] Get the contents (channel values) of a spectrum.
- [[chap7_2_spectrum#spectclspectrumzero]] Clear the contents of spectra.
...
- [[chap7_2_attach#spectclattachattach]] attaches a data source to the server. Note that any previously attached source is detached.
- [[chap7_2_attach#spectclattachlist]] describes the data source attached tot he server.
- [[chap7_2_attach#spectclattachdetach]] detaches the current data source.
...
- [[chap7_2_analyze#spectclanalyzestart]] Starts analysis
- [[chap7_2_analyze#spectclanalyzestop]] Stops analysis
- [[chap7_2_analyze#spectclanalyzesize]] Sets the event chunksize for Rustogramer.
...
- [[chap7_2_apply#spectclapplyapply]] - Applies a gate/condition to one or more spectra.
- [[chap7_2_apply#spectclapplylist]] - Produces a list of gates applied to spectra.
- [[chap7_2_apply#spectclungate]] - Removes any gate a spectrum has.
...
See [[./chap7_2_apply#spectclungate]]
...
- [[chap7_2_channel#spectclchannelset]] allows you to set the value of a channel.
- [[chap7_2_channel#spectclchannelget]] provides the value of a channel
...
- [[chap7_2_evbunpack#spectclevbunpackcreate]] - Creating an event processor with pipeline slots for source ids.
- [[chap7_2_evbunpack#spectclevbunpackadd]] - Associate an existing event processing pipeline with an source id.
- [[chap7_2_evbunpack#spectclevbunpacklist]] - list the event builder event processors that have been created by this command.
...
- **evpname**  (string) - Name of an event processor made via e.g. [[chap7_2_evbunpack#spectclevbunpackadd]].
...
- [[chap7_2_filter#spectclfilternew]] - Which SpecTcl uses to create a new filter.
- [[chap7_2_filter#spectclfilterdelete]] - which delets an existing filter.
- [[chap7_2_filter#spectclfilterenable]] - which enables an existing filter to write it's subset of data for future events.
- [[chap7_2_filter#spectclfilterdisable]] - which disables an existing filter so that it will no longher write events.
- [[chap7_2_filter#spectclfilterregate]] - which associates a different gate with an existing filter, changing the subset of events that will be written by the filter (when enabled).
- [[chap7_2_filter#spectclfilterfile]] - Which specifies a file on which filtered data will be written.
- [[chap7_2_filter#spectclfilterlist]] - which lists filters and their properties.
- [[chap7_2_filter#spectclfilterformat]] - which specifies an output format for a filter.
...
Enables a filter to write events.  Once a file has been associated with a filter it can be enabled to write events to that file. See also [[chap7_2_filter#spectclfilterdisable]]
...
- **format** (string) - The format with which the filter is written. See [[chap7_2_filter#spectclfilterformat]] for more information about this.
...
- [[chap7_2_fit#spectclfitcreate]] - create a new SpecTcl fit object.
- [[chap7_2_fit#spectclfitupdate]] - Computes fit parameter values on current histogram data.
- [[chap7_2_fit#spectclfitdelete]] - Delete a spectcl fit object.
- [[chap7_2_fit#spectclfitlist]] - List one or more fits providing each fit's current function parameterization.
- [[chap7_2_fit#specclfitproc]] - Returns a Tcl proc that can be used to evaluate the fit at any point.
...
- [[chap7_2_fold#spectclfoldapply]] - Apply a condition as a fold to a spectrum
- [[chap7_2_fold#spectclfoldlist]] - List the folds.
- [[chap7_2_fold#spectclfoldremove]] - Remove a fold.
...
- [[chap7_2_shmem#spectclshmemkey]] Get shared memory attachment information
- [[chap7_2_shmem#spectclshmemsize]] Get the total size of the shared memory region.
- [[chap7_2_shmem#spectclshmemvariables]] Provide the values of some "interesting" shared memory variables.
...
- [[chap7_2_sbind#spectclsbindall]] - Bind all spectra to display memory.
- [[chap7_2_sbind#spectclsbindsbind]] - Bind a single spectrum to the display.
- [[chap7_2_sbind#spectclsbindlist]] - List th current bindings.
- [[chap7_2_sbind#spectclsbindset_update]] Rustogramer only, specifies the number of seconds between updates to the shared memory.
- [[chap7_2_sbind#spectclsbindget_update]] Rustogramer only, returns the shared memory refresh rate.
...
- [[chap7_2_ubind#spectclunbindbyname]] - Unbind given the name of a spectrum.
- [[chap7_2_ubind#spectclunbindbyid]] - (SpecTcl only) - by spectrum id.
- [[chap7_2_ubind#spectclunbindall]]  - Unbind all spectra.]
...
- [[chap7_2_pman#spectclpmancreate]] - Create a new, empty, event processing pipeline.
- [[chap7_2_pman#spectclpmanls]] - List just the names of the event procesing pipelines currently defined.
- [[chap7_2_pman#spectclpmancurrent]] - Return the  currently selected pipeline.
- [[chap7_2_pman#spectclpmanlsall]] - List processing pipelines and the event processors in them.
- [[chap7_2_pman#spectclpmanlsevp]] - Lists the names of the event processors.
- [[chap7_2_pman#spectclpmanuse]] - Select the current event processing pipeline.
- [[chap7_2_pman#spectclpmanadd]] - Add an event processor to the end of an event processing pipeline.
- [[chap7_2_pman#spectclpmanrm]] - Remove an event processor from a pipeline.
- [[chap7_2_pman#spectclpmanclear]] - Remove all event processors from a pipe.
- [[chap7_2_pman#spectclpmanclone]] - Create a duplicat of an existing pipeline.
...
Lists just the names of the pipelines. To get more information, see  [[chap7_2_pman#spectclpmanlsall]].
...
- [[chap7_2_pseudo#spectclpseudocreate]] - Create a new pseudo parameter.
- [[chap7_2_pseudo#spectclpseudolist]] - List the properties of pseudo parameters.
- [[chap7_2_pseudo#spectclpseudodelete]] - Delete an exising pseudo parameter.
...
- [[chap7_2_roottree#spectclroottreecreate]] - Makes a new root tree.
- [[chap7_2_roottree#spectclroottreedelete]] - Delete an existing root tree.
- [[chap7_2_roottree#spectclroottreelist]] - List root trees and their properties.
...
- [[chap7_2_treevariable#spectcltreevariablelist]] - List tree variables and their properties.
- [[chap7_2_treevariable#spectcltreevariableset]] - Set new value and units for a tree variable.
- [[chap7_2_treevariable#spectcltreevariablecheck]] - See if the changed flag is set for the tree variable.
- [[chap7_2_treevariable#spectcltreevariablesetchanged]] - Set the changed flag for a treevariable.
- [[chap7_2_treevariable#spectcltreevariablefiretraces]] - Fire the traces for a set of tree variables, allowin scripts and UI elements that care about the variable to know about changes.
...
- Establishes a trace using [[chap7_2_trace#spectcltraceestablish]].  This declares the desire for a client to use the trace system.  The server returns a token the client should use in subsequent trace requests.
- Periodically, the client asks for changes since the last time it asked for changes using [[chap7_2_trace#spectcltracefetch]] supplying its token.
- When the client exits, it ideally issues [[chap7_2_trace#spectcltracedone]] providing its token.  All resources associted with this token are released and the token is rendered invalid.
...
- [[chap7_2_trace#spectcltraceestablish]] is requested first to associated a token with the clietn, and create a trace queue for the client with a retention time.
- [[chap7_2_trace#spectcltracefetch]] - fetches the trace records that have been queued for the client since the last time this request was issued that are not older than the retention time.
- [[chap7_2_trace#spectcltracedone]]  is issued by the client to indicate that it is done using the trace subsystem (usually clients issue this as part of their exit code).
...
- **token** - Value of the token gotten from [[chap7_2_trace#spectcltraceestablish]].
...
- **token** (unsigned) - Mandatory - the token returned when requesting [[chap7_2_trace#spectcltraceestablish]].
...
- [[chap7_mirror#network-messages]] the client and server exchange with each other.
- [[chap7_mirror#client-software]] for your application.
...
- SpecTcl provides a [[chap7_mirror#the-mirrorclient-program]] application (Linux only)
- Both SpecTcl and CutiePie provide a [[chap7_mirror#the-mirror-client-library]] for programs to use (Linux and Windows)
...
Note that once the mirror client program has been run and is maintaining a mirror shared memory, the  [[chap7_mirror#the-mirror-client-library]]  can be used to get a pointer to the mirror in your program.
...
- [[chap7_3#low-level-package]] - a low level obejct oriented interface.
- [[chap7_3#spectcl-command-simulation]] Simulation of SpecTcl commands using the low level REST client software.
...
- *parameters*  Dict containing fit parameters computed by the most recent update request.  THe keys in this dict will depend on the fit type, however all fits should provide a **chisquare** key to asess the goodness of the fit.  See the [[./chap7_2_fit]] for more information.
...
- *pattern* Required glob pattern.  Spectra which match this pattern are cleared.  There is no default, See, however [[chap7_3#spectrumClearAll]] below.  Making pattern required supports clearing individual spectra.
...
Unsets the changed flag of the tree parmaeter (see) [[chap7_3#client-treeparametercheck]] above.
...
This operation creates a new filter defining its name, the condition which must be met to write an event to the filter and a list of parameters that will be written.  Once created a filter must still be [[chap7_3#client-filterfile]] and [[chap7_3#client-filterenable]]for it to write data.
...
Returns the shared memory update period.  See also [[chap7_3#client-shmupdate_set]].
...
Lists the names (only) of all of the registered event processing pipelines.  Pipelines can be registered programmatically or at run time via [[chap7_3#client-pmancreate]], or in user C++ code.
...
- *name*  - name of the event build event processor created with [[chap7_3#client-evbcreate]] above.
...
Lists the event builder event processors (Those reated via [[chap7_3#client-evbcreate]]) with names that match the pattern.
...
2. Periodically, the client polls for new traces by invoking the [[chap7_3#client-tracefetch]] request.
3. When no longer interested in trace data (e.g. on exit) the client performs a [[chap7_3#client-tracedone]] request.
...
[[chap7_3#client-tracefetch]] and [[chap7_3#client-tracedone]] requests.
...
- *token*  - The client token returned from [[chap7_3#client-traceestablish]].
...
- *token* - the token gotten from [[chap7_3#client-traceestablish]].


### [Description](#description-93)


Returns any trace data since the last invokation of traceFetch on this token.  Note that if the time since the last poll was longer than the retention period specified on [[chap7_3#client-traceestablish]] some traces may be lost.
...
- Shows how to [[chap7_3#how-to-get-started-with-an-existing-application]] and
- [[chap7_3#support-for-spectcl-commands]] as well as which ones are not supported by Rustogramer.
...
much of the code below is identical to that of the [[chap7_3#wrapping-an-application-using-source]].
...
[[./chapter_4]] uses this ReST client library.


This section provides:


- Recipies for [[chap7_4#importing-the-client]] on Linux and windows.
- [[chap7_4#rustogramerexception-reference]] exception class.
- [[chap7_4#rustogramer-client-reference]] client class.


## [Importing the client](#importing-the-client)


The issue to consider for being able to import the Python ReST is how to set up the import path given the installation base of the Rustogrammer package.  This is because the winddows and linux installer install these in different subdirectories.  Here is sample code that should work in both Windows and Linux to locate and import both the [[chap7_4#rustogramerexception-reference]] exception class and the [[chap7_4#rustogramer-client-reference]]:
...
- *name* (string) - name of the parameter.
- *properties* (dict) - Properties to modify.  See [[chap7_4#parameter_create]] for a description of this parameter.
...
- *name* (string) - Name of the parameter.
- *properties* (dict) - Metadata for the parameter.  See [[chap7_4#parameter_create]] for a description of the metadata keys.
...
See also [[chap7_4#parameter_uncheck]]
...
Unsets the parameter's check flag.  See [[chap7_4#parameter_check]] for a description of this flag.
...
Create a raw parameter.  This is really different from [[chap7_4#parameter_create]] only for SpecTcl.  Creates a parameter definition and metadata for a parameter that is *not* a tree parameter.  For SpecTcl, this is an important distinction because these parameters:


- Are invisible to [[chap7_4#parameter_list]], however see [[chap7_4#rawparameter_list_by_name]] below.
...
Same as for [[chap7_4#rawparamter_list_byname]].
...
See [[chap7_4#sbind_set_update_period]] for a description of this value.
...
- *chantype* (string) - Channel type specification see [[chap7_4#spectrum_create1d]] for a description of this argument.
...
- *bins* (unsigned > 0) - Number of bins on the X axis.
- *chantype* (string) - data type for bins.  See [[chap7_4#spectrum_create1d]] for more information about this parameter.
...
- `json` format is an ASCII JavaScript Object Notation format.  The schema for that is defined [[./chap7_7]].  This format was developed for Rustogramer and retrofitted in to SpecTcl begininng with version 5.13.
...
Write spectra to file.  See [[chap7_4#spectrum_read]] for the supported file formats.
...
- *format* (string) - File format.  see [[chap7_4#spectrum_read]] for the file formats.
...
When traceable events happen, they are queued in all of the trace queues, and events that are older than the retention time associated with a queue are pruned out of that queue.  This pruning ensures that if a tracing program exits without invoking [[chap7_4#trace_done]], the data in its trace queue will not grow without bounds.


When done tracing or about to exit, a program should invoke [[chap7_4#trace_done]] to release the storage associated with the token and destroy the trace queue.  From time to time, the client should invoke [[chap7_4#trace_fetch]] to obtain the contents of the trace queue.  This should be done more often than the retention period else events can be missed.
...
[[chap7_4#trace_done]] and [[chap7_4#trace_fetch]].
...
- *token* (unsigned) - Token returned from the call to [[chap7_4#trace_establish]].
...
SpecTcl only.  Return the value of the treevariable check flag.  This is set to be true by the [[chap7_4#treevariable_setchanged]] method.  Normally this flag is set when a tree variable is modified.  It allows state change methods to filter the saved tree variable state to only the tree variables that actually changed.
...
SpecTcl only Sets the changed/check flag.  see [[chap7_4#treevariable_check]] for more information aobu this flag.
...
This section documents the database schema of those files.  Note that [[chap7_6#spectcl-schema-additions]] describes the section of the scheme that is only used by SpecTcl.
...
- *save_id - Foreign key to the [[chap7_6#save-sets]] table.  This is the value of the primary key of the row in that table that identifies which save set this parmaeter definition belongs to.
- *name* - Name of the parameter.
...
- *save_id* - is a foreign key into the [[chap7_6#save-sets]] table. This identifies which save set this definition belongs to.
...
- *spectrum_id* is a foreign key into the [[chap7_6#spectrum-definitions]] table indicating which spectrum this axis belongs to.  In a spectrum with two axes, as desdribed above, the one with the smaller value for *id* will be the X axis.
- *low* - Axis low limit.
...
- *spectrum_id* is a foreign key from [[chap7_6#spectrum-definitions]] wich indicates the spectrum this parameter belongs to.
- *parameter_id* is a foreign key from [[chap7_6#parameter-definitions]] indicating the parameter.
...
Note how the joins are used to link the rows in the *spectrum_x_params* tables to the [[chap7_6#spectrum-definitions]] and [[chap7_6#parameter-definitions]] tables via the foreign keys in *spectrum_x_params*
...
- *saveset_id* - Foreign key from [[chap7_6#save-sets]] indicating the save set this definition belongw to.
...
- *gate_id* is a foreign key that contains the primary key of the row in the [[chap7_6#condition-definitions]] table of the gate this point belongs to.
...
- *parent_gate* - A foreign key that identifies which gate in [[chap7_6#condition-definitions]] this parameter belongs to.
- *parameter_id - A foreign key that identifies which parameter in [[chap7_6#parameter-definitions]] this parameter identifies.
...
- *parent_gate* - A foreign key into [[chap7_6#condition-definitions]] identifying which condition, this condition is a component of.
- *child_gate* - A foreign key into [[chap7_6#condition-definitions]] identifying a condition the condition indicated by *parent_gate* depends on.
...
- *parent_gate* - foreign key  in [[chap7_6#condition-definitions]] identifying which condition this mask belongs to.
...
- *spectrum_id* is a foreign key into [[chap7_6#spectrum-definitions]] indicating the spectrum that is being gated.
- *gate_id* is a foreign key into [[chap7_6#condition-definitions]] indicating which condition is the gate.
...
- *id* - is the row primary key.
- *save_id* is a foreign key to [[chap7_6#save-sets]] which indicates the save set that this definition belongs to.
...
- Follow the instructions in [[apppendix_1#doing-the-installation]] above.
