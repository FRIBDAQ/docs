...
know How to look at the rustogramer internals.
...
## [The FRIB analysis pipeline](#the-frib-analysis-pipeline)
...
The key point is that the pipeline processes raw events from some file (e.g. event data acquired at the FRIB) to a simple self-describing data set that Rustogramr can understand without any further processing.  In [[the next section|./chap2_2]], we'll describe the format of the data expected by Rustogramer so that, if you want, you can use whatever other method you want to prepare your data.
...
### [Preparing the parameter definition file](#preparing-the-parameter-definition-file)
...
We're going to ignore that.  Workers that decode raw event data are derived from the `frib::analysis::CMPIRawToParametersWorker`class. Our worker has to construct in a way that it can bind some of its data to the parameters defined in the [[previous section|chap2_1#preparing-the-parameter-definition-file]].  We then must override `unpackData` which actually unpacks a raw event into the parameters.
...
3. The `CTreeParameterArray` class represents a binding between an array like object (implements indexing) of real number like objects and names.  We'll bind this to the names of the paramters we made in our
   [[parameter definition file|chap2_1#preparing-the-parameter-definition-file]]
...
10. The worker, method is called in each worker process. We instantiate the custom worker that we wrote in the [[The worker class|chap2_1#the-worker-class]] section above.
...
[[the next section|chapter_3#using-the-port-manager-to-assign-server-port-values]] below for a description of this process and the prerequisites it requires.
...
[[Using hard coded values|chapter_3#running-rustogramer-with-hard-coded-server-port-values]] above.
...
(see [[The Spectrum Listing|chap4_1#the-spectrum-listing]]).
...
[[Button bar|chap4_1#the-button-bar]]
...
- [[1D spectrum editor|chap4_1#1-d-spectrum]]
- [[2D Spectrum editor|chap4_1#2-d-spectrum]]
- [[Summary Spectra|chap4_1#summary-spectrum]]
- [[Gamma 1D spectra|chap4_1#gamma-1-d-spectrum]]
- [[Gamma 2D spectrum editor|chap4_1#gamma-2-d-spectrum]]
- [[Gamma Deluxe spectrum editor|chap4_1#particle-gamma-spectrum]]
- [[2d Sum Spectrum|chap4_1#2-d-sum-spectrum]]
- [[Projection Spectra|chap4_1#projection-spectra]]
- [[Strip charts|chap4_1#strip-charts]]
- [[Bit mask spectra|chap4_1#bit-mask-parameters]]
- [[Gamma summary spectra|chap4_1#gamma-summary-spectra]]
...
[[summary spectrum|chap4_1#summary-spectrum]], however:  There are two editable parameter lists. Having selected a parameter you can put it in either the X or Y parameters list depending on which of the arrow buttons you click.
...
The editor is identical, however to the [[Gamma Deluxe editor|chap4_1#particle-gamma-spectrum]]:
...
The controls on this tab are similar to those on the [[Parameters tab|./chap4_2]].   The only differences are:
...
- [[Slice editor|chap4_4#slice-conditions]]
- [[Contour editor|chap4_4#contour-condition]]
- [[And and Or editor|chap4_4#and-and-or-conditions]]
- [[Not editor|chap4_4#not-conditions]]
- [[Band editor|chap4_4#band-conditions]]
- [[Gamma band and contour editors|chap4_4#gamma-bands-and-contours]]
- [[Gamma slice editor|chap4_4#gamma-slice-conditions]]
- [[Bit mask conditions|chap4_4#bit-mask-conditions]]
- [[True/false conditions|chap4_4#true-and-false-conditions]]
...
- [[New...|chap4_bindsets#new-bindset]] - makes a new bindset.
- [[Edit Selecteed...|chap4_bindsets#edit-bindset]] - edits the selected bindeset.
...
- [[Save as selected...|chap4_bindsets#save-selected]] - Save the currently bound spectra to the selected bindset
- [[Save as new...|chap4_bindsets#save-new]] - Save the currently bound spectra to a new bindset.
...
the [[New bind set|chap4_bindsets#new-bindset]] section.  However, the editor is first populated with the
...
- [[Save...|chap4_5#file-save]]
- [[Save Treevariables...|chap4_5#file-save-treevariables]] (SpecTcl only)
- [[Save Spectrum contents...|chap4_5#file-save-spectrum-contents]]
- [[Load...|chap4_5#file-load]]
- [[Read spectrum contents...|chap4_5#file-read-spectrum-contents]]
- [[Source Tcl Script...|chap4_5#file-source-tcl-script]] (SpecTcl only)
- [[Exit|chap4_5#file-exit]]
- [[Stop Histogramer|chap4_5#file-stop-histogramer]] (Rustogramer only)
...
SpecTcl only can save tree variable definitions to an Sqlite3 data base file.  The schema for that file is the same as for the file saved by the [[Save configuration|chap4_5#file-save]] operation, however only the tables germane to tree parameter definitions are populated.
...
- File format; as described in [[File->Save spectrum contents...|chap4_5#file-save-spectrum-contents]], the format of the file that is being read.
...
After prompting if you are sure, exits the GUI.  Note that in general the server continues to run and you can connect to it again later with a new GUI instance.  For Rustogramer, see, however [[File->Stop Histogramer|chap4_5#file-stop-histogramer]] below.
...
- [[Online|chap4_6#data-source-online]]  (SpecTcl only) Select an NSCLDAQ helper (e.g. rinselector) and take data from an online system.
- [[File|chap4_6#data-source-file]]  Take data from a file.  Note that as of SpecTcl version 5.13-002,  SpecTcl can analyze data from a parameter file prepared for Rustogramer as well as from raw event files.
- [[Pipe|chap4_6#data-source-pipe]] (SpecTcl only) Read data from an arbitrary helper program.
- [[Cluster File|chap4_6#data-source-cluster-file]] (SpecTcl 5.14 and later only) Use a file to drive analysis from several event files.
- [[Filter file|chap4_6#data-source-filter-file]] (SpecTcl only) take data from a filter file.
- [[Detach|chap4_6#data-source-detach]] Stop analyzing data from the current data source.
- [[Abort Cluster File|chap4_6#data-source-abort-cluster-file]] (SpecTcl only) abort an in progress cluster file.
...
As we have described in [[our documentation of the Data Source->Filter file|./chap4_6#data-source-filter-file-spectcl-only]] menu  SpecTcl an write filtered event data files.  See that section for more information about what filter files are and pointers to SpecTcl documentation on filter files.   The `Filters` menu provides:


- The ability to create filters via a [[filter wizard|chap4_filters#filter-filter-wizard]].
- The ability to [[control which filters are enabled|chap4_filters#filter-enabledisable-filters]]; enabled filters write data.
- Another menu entry to [[attach a filter file|chap4_filters#filter-read-filter-files]] for analyais.
...
- [[Naming the filter|chap4_filters#naming-the-filter]]
- [[Selecting, if desired, a gate|chap4_filters#selecting-the-filter-gate]] to determine which events are written to the  filter file.
- [[Choosing the parameters|chap4_filters#selecting-the-parameters-to-output]] to write for each event that makes the gate true.
- [[Choosing the file|chap4_filters#choosing-the-output-file]] to which filter data will be written and whether the filter should be created
  enabled.
...
the [[Data Source Menu|chap4_6#data-source-filter-file-spectcl-only]].
...
- [[Save spectrum contents.|chap4_7#save-spectrum-contents]]
- [[Read spectrum file|chap4_7#read-spectrum-file]]
- [[Clear All|chap4_7#clear-all]]
- [[Create|chap4_7#create]]
- [[Delete|chap4_7#delete]]
- [[Apply Gate|chap4_7#apply-gate]]
...
# [The Gate Menu](#the-gate-menu)


Allows you to perform the folowing operations on gates.


- [[Create|chap4_8#create-gates]] one or more gates
- [[Apply a gate|chap4_8#apply-gate]] to one or more spectra.
- [[Delete|chap4_8#delete-gate]] a gate.
...
See the various gate editor descriptions in the [[Gate Tab documentation|./chap4_4]]


## [Apply gate](#apply-gate-1)


This functionality is the same as that in [[the Spectrum menu|./chap4_7#apply-gate]]
...
# [Chapter 6 - the REST interface](#chapter-6---the-rest-interface)
...
- [[Tcl|./chap6_1]] which you can use to create/port GUIs using Tcl/Tk
- [[Python|./chap6_2]]  which also includes a sample fully featured [[GUI|./chapter_4]]


To be clear, these are client side APIs.  If you are interested in contributing a binding for another language, [[here is documentation for tha requests and responses|./chap7_2]]
...
[[low level Tcl REST client|chap6_1#using-the-low-level-client]].  To use it you must:
...
1. The `SpecTclRestCommand::initialize` command sets the connection parameters for the pacakge.  The parameters correspond in turn to the `-host` and `-port` option values to the constructor for the `SZpecTclRestClient` described in the [[previous section|chap6_1#using-the-low-level-client]].
...
1. We've extracted the program fragment that shows how to extend the import path [[from the previous section|chap6_2#extending-the-import-path-and-importing-the-module]] into the function `extedn_path` here we call that in the main program.
...
# [Chapter 7 - Reference material](#chapter-7---reference-material)


This chapter contains reference material:


- [[Rustogramer command line options|./chap7_1]]
- [[REST requests and responses|./chap7_2]]
- [[Tcl REST API reference|./chap7_3]]
- [[Python REST API reference|./chap7_4]]
- [[Pointer to the Rustogramer internals documentation|./chap7_5]]
- [[GUI configurationfile schema|./chap7_6]]
- [[Schema of JSON Spectrum contents files|./chap7_7]]
...
# [REST requests and responses](#rest-requests-and-responses)
...
## [Pre-requisites to understand this reference section:](#pre-requisites-to-understand-this-reference-section)
...
- The client, when making requests of SpecTcl must be able to accept data with `Content-Encoding: deflate` as this is used in the resonses to the [[`spectrum/contents` request|./chap7_2_spectrum#spectclspectrumcontents]].
...
- [[`/list`|chap7_2_parameter#spectclparameterlist]] - lists the parameters and their properties.
- [[`/edit`|chap7_2_parameter#spectclparameteredit]] - Modifies the metadata associated with a parameter.
- [[`/promote`|chap7_2_parameter#spectclparameterpromote]] - Promote a raw parameter to a tree parameter.
- [[`/create`|chap7_2_parameter#spectclparametercreate]] - Create a new parameter (Rustogramer only)
- [[`/listnew`|chap7_2_parameter#spectclparameterlistnew]] - lists parameters that have modified metadata.
- [[`/check`|chap7_2_parameter#spectclparametercheck]] - Checks the modified state of parameters.
- [[`/uncheck`|chap7_2_parameter#spectclparameteruncheck]] - Turns off the modified state of parameters.
- [[`/version`|chap7_2_parameter#spectclparameterversion]] - Provides version information about the capabilities of the parameter system (earlier versions of the SpecTcl `treeparameter` did not support the `-create` operation).
...
- Rustogramer - this is equivalent to the [[/spectcl/parameter/edit|chap7_2_parameter#spectclparameteredit]] operation.
...
- [[`/spectcl/rawparameter/new`|chap7_2_rawparameter#spectclrawparameternew]] (Rustogramer maps this to [[`/spectcl/parameter/create`|./chap7_2_parameter#spectclparametercreate]]).  SpecTcl creates a new raw parameter.
- [[`/spectcl/rawparameter/delete`|chap7_2_rawparameter#spectclrawparameterdelete]] (Rustogramer implements this).  This deletes an existing (raw) parameter.
- [[`/spectcl/rawparameter/list`|chap7_2_rawparameter#spectclrawparameterlist]] (Rustogramer maps this to [[`spectcl/parameter/list`|./chap7_2_parameter#spectclparameterlist]])
...
#### [Sample Responses.](#sample-responses-10)
...
- [[`/spectcl/gate/list`|chap7_2_gates#spectclgatelist]] - lists defined conditions.
- [[`/spectcl/gate/delete`|chap7_2_gates#spectclgatedelete]] - Delets a condition
- [[`/spectcl/gate/edit`|chap7_2_gates#spectclgateedit]] - Create or modify a condition.
...
#### [Sample Responses.](#sample-responses-11)
...
#### [Sample Responses.](#sample-responses-12)
...
#### [Sample Responses.](#sample-responses-13)
...
- [[`/spectcl/spectrum/list`|chap7_2_spectrum#spectclspectrumlist]]  List spectra.
- [[`/spectcl/spectrum/delete`|chap7_2_spectrum#spectclspectrumdelete]] Delete an existing spectrum.
- [[`/spectcl/spectrum/create`|chap7_2_spectrum#spectclspectrumcreate]] Create a new spectrum.
- [[`/spectcl/spectrum/contents`|chap7_2_spectrum#spectclspectrumcontents]] Get the contents (channel values) of a spectrum.
- [[`/spectcl/spectrum/zero`|chap7_2_spectrum#spectclspectrumzero]] Clear the contents of spectra.
...
#### [Sample Responses.](#sample-responses-14)
...
#### [Sample Responses.](#sample-responses-15)
...
#### [Sample Responses.](#sample-responses-16)
...
#### [Sample Responses.](#sample-responses-17)
...
### [Response format detail](#response-format-detail-10)


The response is a generic response.


#### [Sample Responses.](#sample-responses-18)
...
- [[`/spectcl/attach/attach`|chap7_2_attach#spectclattachattach]] attaches a data source to the server. Note that any previously attached source is detached.
- [[`/spectcl/attach/list`|chap7_2_attach#spectclattachlist]] describes the data source attached tot he server.
- [[`/spectcl/attach/detach`|chap7_2_attach#spectclattachdetach]] detaches the current data source.
...
### [Response format detail](#response-format-detail-11)


A Generic response is returned.


#### [Sample Responses.](#sample-responses-19)
...
### [Response format detail](#response-format-detail-12)


A generic repsonse.  This always has **status**=`OK`


#### [Sample Responses.](#sample-responses-20)
...
### [Response format detail](#response-format-detail-13)


A generic response.


#### [Sample Responses.](#sample-responses-21)
...
- [[`/spectcl/analyze/start`|chap7_2_analyze#spectclanalyzestart]] Starts analysis
- [[`/spectcl/analyze/stop`|chap7_2_analyze#spectclanalyzestop]] Stops analysis
- [[`/spectcl/analyze/size`|chap7_2_analyze#spectclanalyzesize]] Sets the event chunksize for Rustogramer.
...
### [Response format detail](#response-format-detail-14)
...
### [Response format detail](#response-format-detail-15)


Genric response.


#### [Sample Responses.](#sample-responses-22)
...
### [Response format detail](#response-format-detail-16)
...
- [[`/spectcl/apply/apply`|chap7_2_apply#spectclapplyapply]] - Applies a gate/condition to one or more spectra.
- [[`/spectcl/apply/list`|chap7_2_apply#spectclapplylist]] - Produces a list of gates applied to spectra.
- [[`/spectcl/ungate`|chap7_2_apply#spectclungate]] - Removes any gate a spectrum has.
...
### [Response format detail](#response-format-detail-17)
...
### [Response format detail](#response-format-detail-18)
...
#### [Sample Responses.](#sample-responses-23)
...
### [Response format detail](#response-format-detail-19)


Generic response


#### [Sample Responses.](#sample-responses-24)
...
See [[the apply documentation|./chap7_2_apply#spectclungate]]
...
- [[/spectcl/channel/set|chap7_2_channel#spectclchannelset]] allows you to set the value of a channel.
- [[/spectcl/channel/get|chap7_2_channel#spectclchannelget]] provides the value of a channel
...
### [Response format detail](#response-format-detail-20)


The response is a generic respones.


#### [Sample Responses.](#sample-responses-25)
...
- [[/spectcl/evbunpack/create|chap7_2_evbunpack#spectclevbunpackcreate]] - Creating an event processor with pipeline slots for source ids.
- [[/spectcl/evbunpack/add|chap7_2_evbunpack#spectclevbunpackadd]] - Associate an existing event processing pipeline with an source id.
- [[/spectcl/evbunpack/list|chap7_2_evbunpack#spectclevbunpacklist]] - list the event builder event processors that have been created by this command.
...
### [Response format detail](#response-format-detail-22)


The response is a generic response


#### [Sample Responses.](#sample-responses-27)
...
- **evpname**  (string) - Name of an event processor made via e.g. [[/spectcl/evbunpack/create|chap7_2_evbunpack#spectclevbunpackadd]].
...
### [Response format detail](#response-format-detail-23)
...
#### [Sample Responses.](#sample-responses-28)
...
- [[`new`|chap7_2_filter#spectclfilternew]] - Which SpecTcl uses to create a new filter.
- [[`delete`|chap7_2_filter#spectclfilterdelete]] - which delets an existing filter.
- [[`enable`|chap7_2_filter#spectclfilterenable]] - which enables an existing filter to write it's subset of data for future events.
- [[`disable`|chap7_2_filter#spectclfilterdisable]] - which disables an existing filter so that it will no longher write events.
- [[`regate`|chap7_2_filter#spectclfilterregate]] - which associates a different gate with an existing filter, changing the subset of events that will be written by the filter (when enabled).
- [[`file`|chap7_2_filter#spectclfilterfile]] - Which specifies a file on which filtered data will be written.
- [[`list`|chap7_2_filter#spectclfilterlist]] - which lists filters and their properties.
- [[`format`|chap7_2_filter#spectclfilterformat]] - which specifies an output format for a filter.
...
### [Response format detail](#response-format-detail-25)


The response is a generic respones.


#### [Sample Responses.](#sample-responses-30)
...
### [Response format detail](#response-format-detail-26)


Rsponses are generic responses.


#### [Sample Responses.](#sample-responses-31)
...
Enables a filter to write events.  Once a file has been associated with a filter it can be enabled to write events to that file. See also [[/disable|chap7_2_filter#spectclfilterdisable]]
...
### [Response format detail](#response-format-detail-27)


Generic response.


#### [Sample Responses.](#sample-responses-32)
...
### [Response format detail](#response-format-detail-28)


Generic format.


#### [Sample Responses.](#sample-responses-33)
...
### [Query parameters](#query-parameters-37)
...
### [Response format detail](#response-format-detail-29)


Generic reply.


#### [Sample Responses.](#sample-responses-34)
...
### [Query parameters](#query-parameters-38)
...
### [Response format detail](#response-format-detail-30)


Generic response.


#### [Sample Responses.](#sample-responses-35)
...
### [Query parameters](#query-parameters-39)
...
### [Response format detail](#response-format-detail-31)
...
- **format** (string) - The format with which the filter is written. See [[format|chap7_2_filter#spectclfilterformat]] for more information about this.
...
### [Query parameters](#query-parameters-40)
...
### [Response format detail](#response-format-detail-32)


This is a Generic response.


#### [Sample Responses.](#sample-responses-37)
...
- [[`/spectcl/fit/create`|chap7_2_fit#spectclfitcreate]] - create a new SpecTcl fit object.
- [[`/spectcl/fit/update`|chap7_2_fit#spectclfitupdate]] - Computes fit parameter values on current histogram data.
- [[`/spectcl/fit/delete`|chap7_2_fit#spectclfitdelete]] - Delete a spectcl fit object.
- [[`/spectcl/fit/list`|chap7_2_fit#spectclfitlist]] - List one or more fits providing each fit's current function parameterization.
- [[`/spectcl/fit/proc`|chap7_2_fit#specclfitproc]] - Returns a Tcl proc that can be used to evaluate the fit at any point.
...
### [Query parameters](#query-parameters-41)
...
### [Response format detail](#response-format-detail-33)


The response is a Generic response.


#### [Sample Responses.](#sample-responses-38)
...
### [Query parameters](#query-parameters-42)


- **pattern** (string) - Glob pattern.  Only fits with names that match the pattern are pdated.


### [Response format detail](#response-format-detail-34)


Response is a generic response.


#### [Sample Responses.](#sample-responses-39)
...
#### [Sample Responses.](#sample-responses-40)
...
### [Query parameters](#query-parameters-44)


- **name** - name of the fit.


### [Response format detail](#response-format-detail-36)


A generic response is produced however **detail** is the text of a Tcl proc.


#### [Sample Responses.](#sample-responses-41)
...
- [[`/spectcl/fold/apply`|chap7_2_fold#spectclfoldapply]] - Apply a condition as a fold to a spectrum
- [[`/spectcl/fold/list`|chap7_2_fold#spectclfoldlist]] - List the folds.
- [[`/spectcl/fold/remove`|chap7_2_fold#spectclfoldremove]] - Remove a fold.
...
### [Query parameters](#query-parameters-45)
...
### [Response format detail](#response-format-detail-37)


The response is a generic response.


#### [Sample Responses.](#sample-responses-42)
...
### [Query parameters](#query-parameters-46)
...
### [Response format detail](#response-format-detail-38)
...
#### [Sample Responses.](#sample-responses-43)
...
### [Query parameters](#query-parameters-47)


- **spectrum** (string) Mandatory name of the spetrum that will have folds removed.


### [Response format detail](#response-format-detail-39)


The response is a generic response


#### [Sample Responses.](#sample-responses-44)
...
### [Query parameters](#query-parameters-48)
...
### [Response format detail](#response-format-detail-40)
...
#### [Sample Responses.](#sample-responses-45)
...
- [[`/spectcl/shmem/key`|chap7_2_shmem#spectclshmemkey]] Get shared memory attachment information
- [[`/spectcl/shmem/size`|chap7_2_shmem#spectclshmemsize]] Get the total size of the shared memory region.
- [[`spectcl/shmem/variables`|chap7_2_shmem#spectclshmemvariables]] Provide the values of some "interesting" shared memory variables.
...
### [Query parameters](#query-parameters-49)


None


### [Response format detail](#response-format-detail-41)


The resonse is a generic response.  On success, **detail** will be the shared memory key.


#### [Sample Responses.](#sample-responses-46)
...
### [Query parameters](#query-parameters-50)


None


### [Response format detail](#response-format-detail-42)


*detail* is the stringified size in bytes.


#### [Sample Responses.](#sample-responses-47)
...
### [Query parameters](#query-parameters-51)


No query parameters are  supported.


### [Response format detail](#response-format-detail-43)
...
#### [Sample Responses.](#sample-responses-48)
...
- [[`/spectcl/sbind/all`|chap7_2_sbind#spectclsbindall]] - Bind all spectra to display memory.
- [[`/spectcl/sbind/sbind`|chap7_2_sbind#spectclsbindsbind]] - Bind a single spectrum to the display.
- [[`/spectcl/sbind/list`|chap7_2_sbind#spectclsbindlist]] - List th current bindings.
- [[`/spectcl/sbind/set_update`|chap7_2_sbind#spectclsbindset_update]] Rustogramer only, specifies the number of seconds between updates to the shared memory.
- [[`/spectcl/sbind/get_update`|chap7_2_sbind#spectclsbindget_update]] Rustogramer only, returns the shared memory refresh rate.
...
### [Query parameters](#query-parameters-52)


No paramters are supported.


### [Response format detail](#response-format-detail-44)


A generic response is returned.


#### [Sample Responses.](#sample-responses-49)
...
### [Query parameters](#query-parameters-53)
...
### [Response format detail](#response-format-detail-45)


The response is a generic response.


#### [Sample Responses.](#sample-responses-50)
...
### [Query parameters](#query-parameters-54)
...
### [Response format detail](#response-format-detail-46)
...
#### [Sample Responses.](#sample-responses-51)
...
### [Query parameters](#query-parameters-55)


- **seconds** (unsigned int) - Mandatory.  Provdes a new update period in seconds.


### [Response format detail](#response-format-detail-47)


A generic response.


#### [Sample Responses.](#sample-responses-52)
...
### [Query parameters](#query-parameters-56)


No query parameters are supported.


### [Response format detail](#response-format-detail-48)


The **detail** attribute is an unsigned integer that is the number of seconds between spectrum contents refreshes.


#### [Sample Responses.](#sample-responses-53)
...
For more information about spectrum binding, see [[`/spectcl/sbind`|./chap7_2_sbind]].


This domain of URIs supports a few methods for unbinding spectra.


- [[`/spectcl/unbind/byname`|chap7_2_ubind#spectclunbindbyname]] - Unbind given the name of a spectrum.
- [[`/spectcl/unbind/byid`|chap7_2_ubind#spectclunbindbyid]] - (SpecTcl only) - by spectrum id.
- [[`/spectcl/unbind/all`|chap7_2_ubind#spectclunbindall]]  - Unbind all spectra.]
...
### [Query parameters](#query-parameters-57)


- **name** (string)  - mandatory name of the spetrum to unbind.


### [Response format detail](#response-format-detail-49)


A generic response is produced.


#### [Sample Responses.](#sample-responses-54)
...
### [Query parameters](#query-parameters-58)


- **id** (unsigned) - mandatory parameter that provides the id of the spectrum to unbind.


### [Response format detail](#response-format-detail-50)


Generic response.


#### [Sample Responses.](#sample-responses-55)
...
### [Query parameters](#query-parameters-59)


No query parameters are supported.


### [Response format detail](#response-format-detail-51)


A generic response is returned.


#### [Sample Responses.](#sample-responses-56)
...
SpecTcl maintains a list of all of the mirror clients that have registered with it.  See the [[reference documentation about the mirror server|./chap7_mirror]] for more information about the mirror server protocol in general and mirror registration specifically.
...
### [Query parameters](#query-parameters-60)


No query paramters are supported.


### [Response format detail](#response-format-detail-52)
...
- **shmkey** (string) - Shared memory identifier.  See the [[shmem requests for information about this|./chap7_2_shmem]]
...
- [[`/spectcl/pman/create`|chap7_2_pman#spectclpmancreate]] - Create a new, empty, event processing pipeline.
- [[`/spectcl/pman/ls`|chap7_2_pman#spectclpmanls]] - List just the names of the event procesing pipelines currently defined.
- [[`/spectcl/pman/current`|chap7_2_pman#spectclpmancurrent]] - Return the  currently selected pipeline.
- [[`/spectcl/pman/lsall`|chap7_2_pman#spectclpmanlsall]] - List processing pipelines and the event processors in them.
- [[`/spectcl/pman/lsevp`|chap7_2_pman#spectclpmanlsevp]] - Lists the names of the event processors.
- [[`/spectcl/pman/use`|chap7_2_pman#spectclpmanuse]] - Select the current event processing pipeline.
- [[`/spectcl/pman/add`|chap7_2_pman#spectclpmanadd]] - Add an event processor to the end of an event processing pipeline.
- [[`/spectcl/pman/rm`|chap7_2_pman#spectclpmanrm]] - Remove an event processor from a pipeline.
- [[`/spectcl/pman/clear`|chap7_2_pman#spectclpmanclear]] - Remove all event processors from a pipe.
- [[`/spectcl/pman/clone`|chap7_2_pman#spectclpmanclone]] - Create a duplicat of an existing pipeline.
...
### [Query parameters](#query-parameters-61)


- **name** (string) - Name of the processor to create.


### [Response format detail](#response-format-detail-53)


Generic response


#### [Sample Responses.](#sample-responses-58)
...
Lists just the names of the pipelines. To get more information, see  [[/spectcl/pman/lsall|chap7_2_pman#spectclpmanlsall]].
...
### [Response format detail](#response-format-detail-54)


**detail** is an array of strings.  Each string is the name of a pipeline.


#### [Sample Responses.](#sample-responses-59)
...
### [Query parameters](#query-parameters-63)


No query parameters are supported.


### [Response format detail](#response-format-detail-55)
...
#### [Sample Responses.](#sample-responses-60)
...
### [Query parameters](#query-parameters-64)
...
### [Response format detail](#response-format-detail-56)
...
#### [Sample Responses.](#sample-responses-61)
...
### [Query parameters](#query-parameters-66)


- **name** (string) - Name of the event processing pipeline to make current.


### [Response format detail](#response-format-detail-58)


Generic response.


#### [Sample Responses.](#sample-responses-63)
...
### [Query parameters](#query-parameters-67)
...
### [Response format detail](#response-format-detail-59)


Generic response.


#### [Sample Responses.](#sample-responses-64)
...
### [Query parameters](#query-parameters-68)
...
### [Response format detail](#response-format-detail-60)


The response is a generic response.


#### [Sample Responses.](#sample-responses-65)
...
### [Query parameters](#query-parameters-69)


- **pipeline** (string) - Mandatory name of the pipeline to clear.


### [Response format detail](#response-format-detail-61)


Generates a generic response.


#### [Sample Responses.](#sample-responses-66)
...
### [Query parameters](#query-parameters-70)
...
### [Response format detail](#response-format-detail-62)


A generic response is produced.


#### [Sample Responses.](#sample-responses-67)
...
### [Query parameters](#query-parameters-71)
...
### [Response format detail](#response-format-detail-63)


A generic response is produced.


#### [Sample Responses.](#sample-responses-68)
...
- [[`/spectcl/pseudo/create`|chap7_2_pseudo#spectclpseudocreate]] - Create a new pseudo parameter.
- [[`/spectcl/pseudo/list`|chap7_2_pseudo#spectclpseudolist]] - List the properties of pseudo parameters.
- [[`/spectcl/pseudo/delete`|chap7_2_pseudo#spectclpseudodelete]] - Delete an exising pseudo parameter.


## [/spectcl/pseudo/create](#spectclpseudocreate)


### [Query parameters](#query-parameters-72)
...
### [Response format detail](#response-format-detail-64)


The response is a generic response.


#### [Sample Responses.](#sample-responses-69)
...
### [Query parameters](#query-parameters-73)
...
### [Response format detail](#response-format-detail-65)
...
#### [Sample Responses.](#sample-responses-70)
...
### [Query parameters](#query-parameters-74)


- **name** - Name of the parameter to delete.


### [Response format detail](#response-format-detail-66)


Response is a Generic Response.


#### [Sample Responses.](#sample-responses-71)
...
- [[`/spectcl/roottree/create`|chap7_2_roottree#spectclroottreecreate]] - Makes a new root tree.
- [[`/spectcl/roottree/delete`|chap7_2_roottree#spectclroottreedelete]] - Delete an existing root tree.
- [[`/spectcl/roottree/list`|chap7_2_roottree#spectclroottreelist]] - List root trees and their properties.
...
### [Query parameters](#query-parameters-75)
...
### [Response format detail](#response-format-detail-67)


**detail** is a generic response.


#### [Sample Responses.](#sample-responses-72)
...
### [Query parameters](#query-parameters-76)


- **tree** (string) - Required.  The name of the tree to delete.


### [Response format detail](#response-format-detail-68)


The response is a generic response.


#### [Sample Responses.](#sample-responses-73)
...
### [Query parameters](#query-parameters-77)
...
### [Response format detail](#response-format-detail-69)
...
#### [Sample Responses.](#sample-responses-74)
...
# [/spectcl/script requests](#spectclscript-requests)
...
## [/spectcl/script](#spectclscript)


### [Query parameters](#query-parameters-78)


- **command** (string) - Required.  command string to execute.


### [Response format detail](#response-format-detail-70)


The response generated is a generic response.


#### [Sample Responses.](#sample-responses-75)
...
- [[`/spectcl/treevariable/list`|chap7_2_treevariable#spectcltreevariablelist]] - List tree variables and their properties.
- [[`/spectcl/treevariable/set`|chap7_2_treevariable#spectcltreevariableset]] - Set new value and units for a tree variable.
- [[`/spectcl/treevariable/check`|chap7_2_treevariable#spectcltreevariablecheck]] - See if the changed flag is set for the tree variable.
- [[`/spectcl/treevariable/setchanged`|chap7_2_treevariable#spectcltreevariablesetchanged]] - Set the changed flag for a treevariable.
- [[`/spectcl/treevariable/firetraces`|chap7_2_treevariable#spectcltreevariablefiretraces]] - Fire the traces for a set of tree variables, allowin scripts and UI elements that care about the variable to know about changes.
...
### [Query parameters](#query-parameters-79)


None supported.


### [Response format detail](#response-format-detail-71)
...
#### [Sample Responses.](#sample-responses-76)
...
### [Query parameters](#query-parameters-80)
...
### [Response format detail](#response-format-detail-72)


Generic response.


#### [Sample Responses.](#sample-responses-77)
...
### [Query parameters](#query-parameters-82)


- **name** (string) -  Required. Name of the treevariable whose changed flag will be set.


### [Response format detail](#response-format-detail-74)


Generic response.


#### [Sample Responses.](#sample-responses-79)
...
### [Query parameters](#query-parameters-83)
...
### [Response format detail](#response-format-detail-75)


Generic response


#### [Sample Responses.](#sample-responses-80)
...
# [/spectcl/version requests](#spectclversion-requests)


Provides information about the version and program.


## [/spectcl/version](#spectclversion)
...
### [Query parameters](#query-parameters-84)


none


### [Response format detail](#response-format-detail-76)
...
#### [Sample Responses.](#sample-responses-81)
...
# [/spectcl/exit requests](#spectclexit-requests)
...
# [/spectcl/ringformat request](#spectclringformat-request)
...
## [/spectcl/ringformat](#spectclringformat)


### [Query parameters](#query-parameters-85)
...
### [Response format detail](#response-format-detail-77)


A generic response is returned.


#### [Sample Responses.](#sample-responses-82)
...
### [Query parameters](#query-parameters-86)
...
### [Response format detail](#response-format-detail-78)
...
#### [Sample Responses.](#sample-responses-83)
...
# [/spectcl/swrite requests](#spectclswrite-requests)
...
## [/spectcl/swrite](#spectclswrite)


### [Query parameters](#query-parameters-87)
...
- **format** (string) - Required.  Format in which the file should be written.   Valid format strings are:
  ...
  - `json` - JavaScript Object Notation.  This is supportd by Rustogramer and SpecTcl after version 5.13-012.  For a description of the JSON see [[Format of JSON Spectrum contents files|./chap7_7]].
...
### [Response format detail](#response-format-detail-79)


Response is a Generic Response object.


#### [Sample Responses.](#sample-responses-84)
...
### [Query parameters](#query-parameters-88)
...
### [Response format detail](#response-format-detail-80)


A generic responses is returned.


#### [Sample Responses.](#sample-responses-85)
...
- Establishes a trace using [[`/spectcl/trace/establish`|chap7_2_trace#spectcltraceestablish]].  This declares the desire for a client to use the trace system.  The server returns a token the client should use in subsequent trace requests.
- Periodically, the client asks for changes since the last time it asked for changes using [[`/spectcl/trace/fetch`|chap7_2_trace#spectcltracefetch]] supplying its token.
- When the client exits, it ideally issues [[`/spectcl/trace/done`|chap7_2_trace#spectcltracedone]] providing its token.  All resources associted with this token are released and the token is rendered invalid.
...
- [[`/spectcl/trace/establish`|chap7_2_trace#spectcltraceestablish]] is requested first to associated a token with the clietn, and create a trace queue for the client with a retention time.
- [[`/spectcl/trace/fetch`|chap7_2_trace#spectcltracefetch]] - fetches the trace records that have been queued for the client since the last time this request was issued that are not older than the retention time.
- [[`/spectcl/trace/done`|chap7_2_trace#spectcltracedone]]  is issued by the client to indicate that it is done using the trace subsystem (usually clients issue this as part of their exit code).
...
### [Query parameters](#query-parameters-89)
...
### [Response format detail](#response-format-detail-81)


*detail* is an integer value; the trace token generated to identify the client.


#### [Sample Responses.](#sample-responses-86)
...
- **token** - Value of the token gotten from [[`/spectcl/trace/establish`|chap7_2_trace#spectcltraceestablish]].
...
#### [Sample Responses.](#sample-responses-87)
...
- **token** (unsigned) - Mandatory - the token returned when requesting [[`/spectcl/trace/establish`|chap7_2_trace#spectcltraceestablish]].
...
#### [Sample Responses.](#sample-responses-88)
...
# [Shared memory Mirror service](#shared-memory-mirror-service-1)
...
- [[The network application level protocol|chap7_mirror#network-messages]] the client and server exchange with each other.
- [[The SpecTcl client C++ software available|chap7_mirror#client-software]] for your application.
...
### [**MSG_TYPE_SHMINFO**](#msg_type_shminfo)
...
### [**MSG_TYPE_REQUEST_UPDATE**](#msg_type_request_update)


This messagse has no payload.


### [**MSG_TYPE_FULL_UPDATE** and **MSG_TYPE_PARTIAL_UPDATE**](#msg_type_full_update-and-msg_type_partial_update)
...
## [How client software should work:](#how-client-software-should-work)
...
- SpecTcl provides a [[mirrorclient|chap7_mirror#the-mirrorclient-program]] application (Linux only)
- Both SpecTcl and CutiePie provide a [[mirror client library|chap7_mirror#the-mirror-client-library]] for programs to use (Linux and Windows)
...
Note that once the mirror client program has been run and is maintaining a mirror shared memory, the  [[mirror client library|chap7_mirror#the-mirror-client-library]]  can be used to get a pointer to the mirror in your program.
...
# [Tcl REST reference](#tcl-rest-reference)
...
- [[SpecTclRESTClient|chap7_3#low-level-package]] - a low level obejct oriented interface.
- [[SpecTclRestCommand|chap7_3#spectcl-command-simulation]] Simulation of SpecTcl commands using the low level REST client software.
...
## [Low Level Package](#low-level-package)
...
### [$client applyGate](#client-applygate)
...
### [$client attachSource](#client-attachsource)
...
### [$client attachList](#client-attachlist)
...
### [$client sbindAll](#client-sbindall)
...
### [$client sbindSpectra](#client-sbindspectra)
...
### [$client sbindList](#client-sbindlist)
...
### [$client fitCreate](#client-fitcreate)
...
### [$client fitUpdate](#client-fitupdate)
...
### [$client fitDelete](#client-fitdelete)
...
- *parameters*  Dict containing fit parameters computed by the most recent update request.  THe keys in this dict will depend on the fit type, however all fits should provide a **chisquare** key to asess the goodness of the fit.  See the [[Fit ReST request|./chap7_2_fit]] for more information.
...
### [$client foldApply](#client-foldapply)
...
### [$client foldList](#client-foldlist)
...
### [$client foldRemove](#client-foldremove)
...
### [$client channelGet](#client-channelget)
...
### [$client channelSet](#client-channelset)
...
### [$client spectrumClear](#client-spectrumclear)
...
- *pattern* Required glob pattern.  Spectra which match this pattern are cleared.  There is no default, See, however [[spectrumClearAll|chap7_3#spectrumClearAll]] below.  Making pattern required supports clearing individual spectra.
...
### [$client spectrumClearAll](#client-spectrumclearall)
...
### [$client spectrumProject](#client-spectrumproject)
...
### [$client spectrumStatistics](#client-spectrumstatistics)
...
### [$client treeparameterCreate](#client-treeparametercreate)
...
### [$client treeparameterList](#client-treeparameterlist)
...
### [$client treeparameterListNew](#client-treeparameterlistnew)
...
### [$client treeparameterSet](#client-treeparameterset)
...
### [$client treeparameterSetInc](#client-treeparametersetinc)
...
### [$client treeparameterSetBins](#client-treeparametersetbins)
...
### [$client treeparameterSetUnits](#client-treeparametersetunits)
...
### [$client treeparameterSetLimits](#client-treeparametersetlimits)
...
### [$client treeparameterCheck](#client-treeparametercheck)
...
### [treeparameterUncheck](#treeparameteruncheck)
...
Unsets the changed flag of the tree parmaeter (see) [[treeparameterCheck|chap7_3#client-treeparametercheck]] above.
...
### [$client treeparameterVersion](#client-treeparameterversion)
...
### [$client treevariableList](#client-treevariablelist)
...
### [$client treevariableSet](#client-treevariableset)
...
### [$client treevariableCheck](#client-treevariablecheck)
...
### [$client treevariableSetChanged](#client-treevariablesetchanged)
...
### [$client treevariableFireTraces](#client-treevariablefiretraces)
...
### [$client filterCreate](#client-filtercreate)
...
This operation creates a new filter defining its name, the condition which must be met to write an event to the filter and a list of parameters that will be written.  Once created a filter must still be [[associated with an output file|chap7_3#client-filterfile]] and [[enabled|chap7_3#client-filterenable]]for it to write data.
...
### [$client filterDelete](#client-filterdelete)
...
### [$client filterEnable](#client-filterenable)
...
### [$client filterDisable](#client-filterdisable)
...
### [$client filterRegate](#client-filterregate)
...
### [$client filterFile](#client-filterfile)
...
### [$client filterList](#client-filterlist)
...
### [$client filterFormmat](#client-filterformmat)
...
### [$client gateList](#client-gatelist)
...
### [$client gateDelete](#client-gatedelete)
...
### [gateCreateSimple1D](#gatecreatesimple1d)
...
### [$client gateCreateSimple2D](#client-gatecreatesimple2d)
...
### [$client gateCreateMask](#client-gatecreatemask)
...
### [$client createCompound](#client-createcompound)
...
Creates a gate that depends on other gates.  These are `*`, `+````, `-`and`c2band`.   For a `-`gate, only one gate name can be in *gate*.  For a`g2band`gate, there must be two dependent gates and they must both be`b``` gates.
...
### [$client integrate](#client-integrate)
...
### [$client parameterNew](#client-parameternew)
...
### [$client parameterDelete](#client-parameterdelete)
...
### [$client parameterList](#client-parameterlist)
...
### [$client pseudoCreate](#client-pseudocreate)
...
### [$client pseudoList](#client-pseudolist)
...
### [$client pseudoDelete](#client-pseudodelete)
...
### [$client ringformat](#client-ringformat)
...
### [$client scontents](#client-scontents)
...
### [$client shmemsize](#client-shmemsize)
...
### [$client shmupdate_set](#client-shmupdate_set)
...
### [$client shmupdate_get](#client-shmupdate_get)
...
Returns the shared memory update period.  See also [[shm_set|chap7_3#client-shmupdate_set]].
...
### [$client spectrumList](#client-spectrumlist)
...
### [$client spectrumCreate](#client-spectrumcreate)
...
### [$client spectrumDelete](#client-spectrumdelete)
...
### [$client unbindByName](#client-unbindbyname)
...
### [$client unbindById](#client-unbindbyid)
...
### [$client unbindAll](#client-unbindall)
...
### [$client rootTreeCreate](#client-roottreecreate)
...
### [$client rootTreeDelete](#client-roottreedelete)
...
### [$client rootTreeList](#client-roottreelist)
...
### [$client pmanCreate](#client-pmancreate)
...
### [$client pmanList](#client-pmanlist)
...
Lists the names (only) of all of the registered event processing pipelines.  Pipelines can be registered programmatically or at run time via [[pmanCreate|chap7_3#client-pmancreate]], or in user C++ code.
...
### [$client pmanListAll](#client-pmanlistall)
...
### [$client pmanListEventProcessors](#client-pmanlisteventprocessors)
...
### [$client pmanRemove](#client-pmanremove)
...
### [$client pmanClear](#client-pmanclear)
...
### [$client pmanClone](#client-pmanclone)
...
### [$client evbCreate](#client-evbcreate)
...
- *name*  - name of the event build event processor created with [[evbCreate|chap7_3#client-evbcreate]] above.
...
Lists the event builder event processors (Those reated via [[evbCreate|chap7_3#client-evbcreate]]) with names that match the pattern.
...
### [$client traceEstablish](#client-traceestablish)
...
2. Periodically, the client polls for new traces by invoking the [[traceFetch|chap7_3#client-tracefetch]] request.
3. When no longer interested in trace data (e.g. on exit) the client performs a [[traceDone|chap7_3#client-tracedone]] request.
...
[[tracFetch|chap7_3#client-tracefetch]] and [[traceDone|chap7_3#client-tracedone]] requests.
...
### [$client traceDone](#client-tracedone)
...
- *token*  - The client token returned from [[traceEstablish|chap7_3#client-traceestablish]].
...
### [$client traceFetch](#client-tracefetch)
...
- *token* - the token gotten from [[traceEstablish|chap7_3#client-traceestablish]].


### [Description](#description-93)


Returns any trace data since the last invokation of traceFetch on this token.  Note that if the time since the last poll was longer than the retention period specified on [[traceEstablish|chap7_3#client-traceestablish]] some traces may be lost.
...
## [SpecTcl Command Simulation](#spectcl-command-simulation)
...
- Shows how to [[start up an application that use  SpecTcl command simulation|chap7_3#how-to-get-started-with-an-existing-application]] and
- [[Describes the set of supported commands|chap7_3#support-for-spectcl-commands]] as well as which ones are not supported by Rustogramer.
...
#### [Wrapping an application using `source`](#wrapping-an-application-using-source)
...
#### [Wrapping an application using `package require`](#wrapping-an-application-using-package-require)
...
much of the code below is identical to that of the [[previous example|chap7_3#wrapping-an-application-using-source]].
...
### [Support for SpecTcl commands:](#support-for-spectcl-commands)
...
# [Python REST reference](#python-rest-reference)
...
[[GUI|./chapter_4]] uses this ReST client library.


This section provides:


- Recipies for [[Importing the ReST client|chap7_4#importing-the-client]] on Linux and windows.
- [[Reference for the RustogramerException|chap7_4#rustogramerexception-reference]] exception class.
- [[Reference for the rustogramer|chap7_4#rustogramer-client-reference]] client class.


## [Importing the client](#importing-the-client)


The issue to consider for being able to import the Python ReST is how to set up the import path given the installation base of the Rustogrammer package.  This is because the winddows and linux installer install these in different subdirectories.  Here is sample code that should work in both Windows and Linux to locate and import both the [[RustogramerException|chap7_4#rustogramerexception-reference]] exception class and the [[client class|chap7_4#rustogramer-client-reference]]:
...
## [RustogramerException Reference](#rustogramerexception-reference)
...
### [ungate_spectrum](#ungate_spectrum)
...
### [evbunpack_create](#evbunpack_create)
...
### [filter_setformat](#filter_setformat)
...
### [condition_make_true](#condition_make_true)
...
### [condition_make_false](#condition_make_false)
...
### [conditio_make_not](#conditio_make_not)
...
### [condition_make_or](#condition_make_or)
...
### [condition_make_and](#condition_make_and)
...
### [condition_make_slice](#condition_make_slice)
...
### [condition_make_contour](#condition_make_contour)
...
### [condition_make_band](#condition_make_band)
...
### [condition_make_gamma_slice](#condition_make_gamma_slice)
...
### [condition _make_gamma_contour](#condition-_make_gamma_contour)
...
### [condition_make_gamma_band](#condition_make_gamma_band)
...
### [condition_make_mask_equal](#condition_make_mask_equal)
...
### [condition_make_mask_and](#condition_make_mask_and)
...
### [condition_make_mask_equal](#condition_make_mask_equal-1)
...
### [condition_make_mask_nand](#condition_make_mask_nand)
...
- *name* (string) - name of the parameter.
- *properties* (dict) - Properties to modify.  See [[parameter_create|chap7_4#parameter_create]] for a description of this parameter.
...
### [parameter_promote](#parameter_promote)
...
- *name* (string) - Name of the parameter.
- *properties* (dict) - Metadata for the parameter.  See [[parameter_create|chap7_4#parameter_create]] for a description of the metadata keys.
...
See also [[parameter_uncheck|chap7_4#parameter_uncheck]]
...
### [parameter_uncheck](#parameter_uncheck)


#### [Description](#description-151)


Unsets the parameter's check flag.  See [[parameter_check|chap7_4#parameter_check]] for a description of this flag.
...
### [rawparameter_create](#rawparameter_create)


#### [Description](#description-152)


Create a raw parameter.  This is really different from [[parameter_create|chap7_4#parameter_create]] only for SpecTcl.  Creates a parameter definition and metadata for a parameter that is *not* a tree parameter.  For SpecTcl, this is an important distinction because these parameters:


- Are invisible to [[parameter_list|chap7_4#parameter_list]], however see [[rawparameter_list_by_name|chap7_4#rawparameter_list_by_name]] below.
...
### [rawparamter_list_byname](#rawparamter_list_byname)
...
### [rawparameter_list_byid](#rawparameter_list_byid)
...
Same as for [[rawparameter_list_byname|chap7_4#rawparamter_list_byname]].
...
### [sbind_set_update_period](#sbind_set_update_period)
...
### [sbind_get_update_period](#sbind_get_update_period)
...
See [[sbind_set_update_period|chap7_4#sbind_set_update_period]] for a description of this value.
...
[[unbind_by_names|chap7_4#unbind_by_names]].
...
### [shmem_getvariables](#shmem_getvariables)
...
### [spectrum_delete](#spectrum_delete)
...
### [spectrum_create1d](#spectrum_create1d)
...
### [spectrum_create2d](#spectrum_create2d)
...
- *chantype* (string) - Channel type specification see [[spectrum_create1d|chap7_4#spectrum_create1d]] for a description of this argument.
...
### [spectrum_createg1](#spectrum_createg1)
...
- *bins* (unsigned > 0) - Number of bins on the X axis.
- *chantype* (string) - data type for bins.  See [[spectrum_create1d|chap7_4#spectrum_create1d]] for more information about this parameter.
...
### [spectrum_createg2](#spectrum_createg2)
...
### [spectrum_creategd](#spectrum_creategd)
...
### [spectrum_createsummary](#spectrum_createsummary)
...
### [spectrum_create2dsum](#spectrum_create2dsum)
...
### [spectrum_createstripchart](#spectrum_createstripchart)
...
### [spectrum_createbitmask](#spectrum_createbitmask)
...
### [spectrum_creategammasummary](#spectrum_creategammasummary)
...
### [spectrum_getcontents](#spectrum_getcontents)
...
### [spectrum_clear, spectrum_clear_all](#spectrum_clear-spectrum_clear_all)
...
- `json` format is an ASCII JavaScript Object Notation format.  The schema for that is defined [[later in this reference material|./chap7_7]].  This format was developed for Rustogramer and retrofitted in to SpecTcl begininng with version 5.13.
...
### [spectrum_write](#spectrum_write)


#### [Description](#description-183)


Write spectra to file.  See [[spectrum_read|chap7_4#spectrum_read]] for the supported file formats.
...
- *format* (string) - File format.  see [[spectrum_read|chap7_4#spectrum_read]] for the file formats.
...
### [pipeline_list_details](#pipeline_list_details)
...
### [pipeline_add_processor](#pipeline_add_processor)
...
### [pipeline_remove_processor](#pipeline_remove_processor)
...
### [roottree_create](#roottree_create)
...
### [roottree_delete](#roottree_delete)
...
### [trace_establish](#trace_establish)
...
When traceable events happen, they are queued in all of the trace queues, and events that are older than the retention time associated with a queue are pruned out of that queue.  This pruning ensures that if a tracing program exits without invoking [[trace_done|chap7_4#trace_done]], the data in its trace queue will not grow without bounds.


When done tracing or about to exit, a program should invoke [[trace_done|chap7_4#trace_done]] to release the storage associated with the token and destroy the trace queue.  From time to time, the client should invoke [[trace_fetch|chap7_4#trace_fetch]] to obtain the contents of the trace queue.  This should be done more often than the retention period else events can be missed.
...
[[trace_done|chap7_4#trace_done]] and [[trace_fetch|chap7_4#trace_fetch]].
...
- *token* (unsigned) - Token returned from the call to [[trace_establish|chap7_4#trace_establish]].
...
### [treevariable_list](#treevariable_list)
...
### [treevariable_set](#treevariable_set)
...
### [treevariable_check](#treevariable_check)


#### [Description](#description-210)


SpecTcl only.  Return the value of the treevariable check flag.  This is set to be true by the [[treevariable_setchanged|chap7_4#treevariable_setchanged]] method.  Normally this flag is set when a tree variable is modified.  It allows state change methods to filter the saved tree variable state to only the tree variables that actually changed.
...
### [treevariable_setchanged](#treevariable_setchanged)


#### [Description](#description-211)


SpecTcl only Sets the changed/check flag.  see [[treevariable_check|chap7_4#treevariable_check]] for more information aobu this flag.
...
### [treevariable_firetraces](#treevariable_firetraces)
...
### [kill_histogramer](#kill_histogramer)
...
# [Looking at the Rustogramer internals documentation](#looking-at-the-rustogramer-internals-documentation)
...
This section documents the database schema of those files.  Note that [[SpecTcl schema additions|chap7_6#spectcl-schema-additions]] describes the section of the scheme that is only used by SpecTcl.
...
- *save_id - Foreign key to the [[save_sets|chap7_6#save-sets]] table.  This is the value of the primary key of the row in that table that identifies which save set this parmaeter definition belongs to.
- *name* - Name of the parameter.
...
- *save_id* - is a foreign key into the [[save_sets|chap7_6#save-sets]] table. This identifies which save set this definition belongs to.
...
- *spectrum_id* is a foreign key into the [[spectrum_defs|chap7_6#spectrum-definitions]] table indicating which spectrum this axis belongs to.  In a spectrum with two axes, as desdribed above, the one with the smaller value for *id* will be the X axis.
- *low* - Axis low limit.
...
- *spectrum_id* is a foreign key from [[spectrum_defs|chap7_6#spectrum-definitions]] wich indicates the spectrum this parameter belongs to.
- *parameter_id* is a foreign key from [[parameter_defs|chap7_6#parameter-definitions]] indicating the parameter.
...
Note how the joins are used to link the rows in the *spectrum_x_params* tables to the [[spectrum_defs|chap7_6#spectrum-definitions]] and [[parameter_defs|chap7_6#parameter-definitions]] tables via the foreign keys in *spectrum_x_params*
...
- *saveset_id* - Foreign key from [[save_sets|chap7_6#save-sets]] indicating the save set this definition belongw to.
...
- *gate_id* is a foreign key that contains the primary key of the row in the [[gate_defs|chap7_6#condition-definitions]] table of the gate this point belongs to.
...
- *parent_gate* - A foreign key that identifies which gate in [[gate_defs|chap7_6#condition-definitions]] this parameter belongs to.
- *parameter_id - A foreign key that identifies which parameter in [[parameter_defs|chap7_6#parameter-definitions]] this parameter identifies.
...
- *parent_gate* - A foreign key into [[gate_defs|chap7_6#condition-definitions]] identifying which condition, this condition is a component of.
- *child_gate* - A foreign key into [[gate_defs|chap7_6#condition-definitions]] identifying a condition the condition indicated by *parent_gate* depends on.
...
- *parent_gate* - foreign key  in [[gate_defs|chap7_6#condition-definitions]] identifying which condition this mask belongs to.
...
- *spectrum_id* is a foreign key into [[spectrum_defs|chap7_6#spectrum-definitions]] indicating the spectrum that is being gated.
- *gate_id* is a foreign key into [[gate_defs|chap7_6#condition-definitions]] indicating which condition is the gate.
...
- *id* - is the row primary key.
- *save_id* is a foreign key to [[save_sets|chap7_6#save-sets]] which indicates the save set that this definition belongs to.
...
- Follow the instructions in [[Doing the installation|apppendix_1#doing-the-installation]] above.
