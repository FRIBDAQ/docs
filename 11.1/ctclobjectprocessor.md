...
`   void  bindAll(CTCLInterpreter&  interp, std::vector<CTCLObject>&  objv);`
            Bind the `interp` to all of the objects in `objv`.
            This is normally used by command processors to bind their command words.  This
            is necessary because there are some operations on
            `CTCLObject` objects that are only possible if they've
            been bound to an interpreter.

`  const   void  requireAtLeast  (std::vector<CTCLObject>&  objv, unsigned  n, const char*  msg  = 0)          throws std::string;`
            Throws a `std::string` exception if the `objv` array
            has fewer than `n` elements.  If
            `msg` is not null it is included in the
            string that is thrown.

`  const void   requireAtMost(std::vector<CTCLObject>&  objv, unsigned  n, const  char*  msg = 0)          throws std::string;`
            Same as `requireAtLeast` but throws when
            `objv` has more than `n`
            elements.

`   const void  requireExactly(std::vector<CTCLObject>&  objv, unsigned  n, const  char*  msg = 0)          throws std::string;`
            Same as `requireAtLeast` but throws if
            `objv` does not have eactly
            `n` elements.

<a name="AEN39147"></a>## SEE ALSO
