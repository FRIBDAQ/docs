...
<a name="AEN12659"></a>## Name

eventlog -- Record Event Data to Disk.

<a name="AEN12662"></a>## Synopsis

**eventlog [options...]
	**

<a name="AEN12666"></a>## DESCRIPTION
...
<a name="AEN12676"></a>## OPTIONS



`--checksum`If present, an SHA512 checksum is computed over the data in each run
...
`--number-of-sources`=*n*Indicates to the logger the number of end of run items it should
...
`--run` *number*When present, the `number` unconditionally sets
...
`--combine-runs`When present, the normal checks in `--one-shot` mode
