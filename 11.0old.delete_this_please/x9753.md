...
[[`CPasswordCheck`|r36123]]The entity must provide a correct password.  Note that the current
                    set of interactors do not support encrypted interactors.  This can be
                    extended if required.

[[`CUnixUserCheck`|r36311]]The entity must provide a valid username and password
                        that is could login to the local unix system.

[[`CTclAccesListCheck`|r36522]]Intended for use within a Tcl interpreter.  The entity must
                        supply some string that is an element of a Tcl list
                        held in a Tcl variable.

[[`CAccessListCheck`|r36620]]The entity must supply a string that is one of a set of strings
                            given to the authenticator.

[[`CHostListCheck`|r36762]]Same as `CAccessListCheck` but the
