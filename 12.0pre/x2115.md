...
# <a name="vmusb-develop-tstamplib"></a>5.7. Developing a Timestamp Extractor Library

As was mentioned in [[x1939]], the user can pass a
...
## <a name="vmusb-develop-getEventTstamp"></a>5.7.1. Implementing a getEventTimestamp() function
...
<a name="AEN2126"></a>`uint64_t getEventTimestamp(void *pBuffer);`
...
<a name="AEN2135"></a>**Example 5-3. A sample getEventTimestamp() implementation**
...
From the [[x2109]], we know the first
...
## <a name="vmusb-develop-getSclrTstamp"></a>5.7.2. Implementing the getScalerTimestamp() function
...
Sometimes it may be useful to label the scaler data with a timestamp as
        ...
        implementing the scaler timestamp (these technically also apply to the
        event timestamp):



1. Always return NULL_TIMESTAMP
2. Returning computed value

If you have implemented a timestamp extractor for event data and you do
       not care about having a precise timestamp labeling your scaler data, the
       first option is a good one. The NULL_TIMESTAMP value as defined in the 
       DataFormat.h file is treated specially by the
...
<a name="AEN2181"></a>`uint64_t getScalerTimestamp(void *pBuffer);`

<a name="AEN2187"></a>**Example 5-4. 
...
Because the DataFormat.h header has been included, the compiler
          will need to be told where it lives. That file lives in the $DAQROOT/include
          directory. So if you add "-I$DAQROOT/include" to your compilation
          recipe, then you should be fine.

## <a name="AEN2192"></a>5.7.3. Implementing an onBeginRun() function

The third function that can be defined in the timestamp extractor
        library has the prototype:

<a name="AEN2195"></a>`void onBeginRun(void);`
...
<a name="AEN2201"></a>**Example 5-5. Adding an offset to the timestamp**
...
### <a name="AEN2224"></a>5.7.3.1. Compiling the shared library
...
```
          spdaqXX> g++ -std=c++11 -fPIC -shared -I$DAQROOT/include -o mytstamplib.so mytstamplib.cpp
        
```

It has been assumed that the DAQROOT environment variable has been set. It 
          gets set by sourcing the daqsetup.bash script in the installation direcotry
          of your NSCLDAQ version. At the NSCL, this is going to look something like:

```
          spdaqXX> source /usr/opt/nscldaq/11.2-003/daqsetup.bash -f
        
```

The **-f** option forces any existing version to be overridden by the script. More
          often than not, it is the desired behavior.
