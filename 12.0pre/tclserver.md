...
<a name="AEN91631"></a>## Name

TclServer -- Embeddable Tcl Server script object

<a name="AEN91634"></a>## Synopsis
...
<a name="AEN91657"></a>## OPTIONS

Several options control the way the server works:



`-port` *port-num*Determines which TCP/IP port the server will listen on
...
`-onconnect` *script*Specifies a script to be called when a connection
...
`-onerror` *script*Specifies a script that will be called whenever a remotely
...
<a name="AEN91701"></a>## METHODS

The following methods control the server.



`start`Starts the server.  This puts the server into the
...
`stop`Stops the server.  This puts the server into the
...
`close`Closes all active client connections.   This can be
