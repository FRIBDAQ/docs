---
---

[← end](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r70449.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [begin →](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r70471.md)

---

---

# <a name="provider3_capabilities"></a>capabilities

<a name="AEN70472"></a>## Name

capabilities -- Get Provider Capabilities Dict

<a name="AEN70475"></a>## Synopsis

**
proc ::*providerName*::capabilities {} {
...
}
          **

<a name="AEN70479"></a>## DESCRIPTION

Not all data source providers are created equal.  This command
            is expected to return a dict that describes the data source's
            capabilities.  The keys to this dict are capability names.
            The values generally are booleans that indicate whether that
            capability is supported.

<a name="AEN70482"></a>## CAPABILITIES

At present the following capabilities are defined:



- canPause
  If true the provider implements the
                              **pause** and
                              **resume** procs and they
                              do what they are advertised to do.
- runsHaveTitles
  If true the provider will honor the
                              `title` parameter passed to the
                              [*begin*](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r70299.md)
                              command.
- runsHaveNumbers
  If true the provider will honer the
                              `runNumber` parameter
                              passed to the
                              [*begin*](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r70299.md)
                              command.

---

---

[← end](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r70449.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [begin →](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r70471.md)
