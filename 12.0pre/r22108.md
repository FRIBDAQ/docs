---
---

[← 1sbsReadout](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r22107.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [Readout →](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r22109.md)

---

---

# <a name="manpage.bcnaf"></a>bcnaf

<a name="AEN22122"></a>## Name

bcnaf -- bcnaf via SBS VME CAMAC interfaces

<a name="AEN22125"></a>## Synopsis

**
bcnaf `-ces | -wiener` *b c n a f* [d]
    **

<a name="AEN22131"></a>## DESCRIPTION

Performs the indicated CAMAC operation. The operation is defined by the
        `b, c, n, a, f` parameters that provide the CAMAC
        branch, crate, slot, subaddress and function code respectively. The
        optional `d` parameter provides the data for
        write operations (it is actually required for CAMAC write operations).

The command writes the data, Q and X responses to stdout.

<a name="AEN22137"></a>## OPTIONS



- `-ces`
  Selects the CES CBD 8210 CAMAC Branch Highway Driver as the
                      CAMAC controller.
- `-wiener`
  Selects the Weiner VC32/CC32 board set as the CAMAC
                      controller.

<a name="AEN22150"></a>## SEE ALSO

[cesbcnaf(1tcl)](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r22038.md),

---

---

[← 1sbsReadout](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r22107.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [Readout →](https://github.com/FRIBDAQ/docs/tree/main/12.0pre/r22109.md)
