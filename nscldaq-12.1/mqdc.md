...
<a name="AEN87281"></a>## Name

mqdc -- Support Mesytec MQDC-32 modules

<a name="AEN87284"></a>## Synopsis
...
`-base` *address*`address` must be the module base
...
`-timestamp` *onoff*This option controls whether or not the
...
`-usethresholds` *onoff*The *onoff* sets whether the thresholds will be
                  used or not. The effect is the same as providing a list of 32 zeroes to the
                  `-thresholds` option.

By default, the value is true.

`-thresholds` *valueList*Supplies the per channel thresholds for the adc.
...
`-ipl` *irqlevel*If the module will be used to trigger an interrupt driven
...
`-vector` *statusId*If the module will be used to trigger an interrupt driven
...
Note that while the MQDC has an 8 bit statusID, the
		    ...
		    a `-vector` 0x80
		    in an MQDC will produce a statusID of 0xff80
		    as seen by the VMUSB.

`-irqthreshold` *integer*Sets the interrupt threshold.  If interrupts are enabled, the module
...
`-multievent` *off | on | limited*Configures the device for multievent mode. The valid values are off,
		  on, and limited. When the value is off,
                  ...
                  read out an amount determined by the `-maxtransfers` option.
                  The default value for this parameter is off.

`-countevents` `on | off`If the value passed to this parameter is on, then the `-maxtransfers`
                  ...
                  only affects the output when the `-multievent` parameter
                  is set to limited. The default value is no.

`-skipberr` `yes | no`If true, then an "end of buffer" is sent instead of a BERR (0xffffffff) when the 
...
`-maxtransfers` `integer`This is a soft threshold for the number of words that can be transferred in a single
                  read when the `-multievent` parameter is set to limited. The number of words 
	          transferred is the least number of words beyond the value set to complete an event. This is
                  only a meaningful parameter if the `-multievent` parameter is set to limited.
                  The default value is 1.

`-bankoffsets` *intlist*The *intlist* must be a two element TCL list whose
...
`-gatemode` *mode*The MQDC32 has a pair of gate inputs.  The *mode*
...
`-gatelimits` *intlist*The *intlist* must be a two element TCL list
...
`-exptrigdelays` *intlist*The *intlist* must be a two element TCL list
                  that contains integer elements each in the range [0,16383]. The range maps to
                  a range of delay from 0 ns to 16384 ns.

The default value is 0.

`-inputcoupling0` *type*The *type* determines whether or not the signal inputs
                  for bank 0 are AC or DC couplied. The accepted values for the option are
                  AC and DC.

The default value is AC.

`-inputcoupling1` *type*This has the exact same semantics as the `-inputcoupling0` option 
                  except that it applies to the bank 1 signal inputs.

The default value is AC.

`-pulser` *onoff*The *onoff* specifies whether the internal test
...
`-pulseramp` *value*The integer value provided for *value* is to be
                  used for the pulser amplitude. The value must be in the range [0,255].

The default value 32.

`-ecltermination` *onoff*This parameter when true enables the ECL input termination.
...
`-ecltiming` *onoff*When true, this parameter enables the gate1 ECL input to
...
If this option is set with `-customecl` is false, Bit 1 of
                  ts_sources register (0x6096) is automatically set to 1.

The default value is false.

`-eclgate1osc` *0,1*0 - Gate 1 input, 1 - Oscillator input.

Setting this option to 1 automatically sets Bit 1 of ts_sources register
                  (0x6096) to 1.

Only valid when `-customecl` is `true`.
                  The default value is 0.

`-eclfcreset` *0,1,2*0 - Fast clear input(="not Gate"), 1 - Reset timestamp counter,
                  2 - input for experiment trigger.

Only valid when `-customecl` is `true`.
                  The default value is 0.

`-customecl` *onoff*When true, ECL_gate1_osc and ECL_FC_Reset registers become customizable.
                  In this case, `-ecltiming` will be ignored regardless of
                  the value set.

The default value is false.

`-nimtiming` *onoff*If true, enables the NIM Gate1 input to be a clock source
...
If this option is set with `-customnim` is false, Bit 1 of
                  ts_sources register (0x6096) is automatically set to 1.

The default value is false.

`-nimgate1osc` *0,1*0 - Gate 1 input, 1 - Oscillator input.

Setting this option to 1 automatically sets Bit 1 of ts_sources register
                  (0x6096) to 1.

Only valid when `-customnim` is true.
                  The default value is 0.

`-nimfcreset` *0,1,2*0 - Fast clear input(="not Gate"), 1 - Reset timestamp counter,
                  2 - input for experiment trigger.

Only valid when `-customnim` is true.
                  The default value is 0.

`-customnim` *onoff*When true, NIM_gate1_osc and NIM_FC_Reset registers become customizable.
                  In this case, `-nimtiming` will be ignored regardless of
                  the value set.

The default value if false.

`-nimbusy` *busyselect*This option selects which signal is presented at the
...
busyThe module busy is outputted.  This is the default.

rcbusThis allows the device to be used as a proxy for the Mesytec RC-bus
...
overthresholdA gate will be outputted when the buffer contains more data words than 
...
fullA gate will be outputted when the buffer is full of data.

`-timingsource` *sourceName*The value of *sourceName* specifies the 
...
`-timingdivisor` *val*The val is a 16-bit integer that scales down the 
                  counter in use (event count or timestamp).

The default value is 1.

`-resetlogic` *mode*The effect of this option should not be confused with enabling or disabling
...
neverIn this mode, the counters will never reset automatically between
                        runs. The only 
...
begin_runIn this mode, the counters will reset during the intialization phase
                        of every run. If using a trigger count (aka. an event count), this is 
                        the preferred reset logic.

extern_oneshotIn this mode, the counter will never reset automatically between runs
...
`-multlowerlimit0` *val*The *val* must be an integer in the range [0,32]. 
...
`-multlowerlimit1` *val*The *val* must be an integer in the range [0,16]. 
                  The effect of this option changes 
...
`-multupperlimit0` *val*The requirements and meaning for this are the same as for the 
...
`-multupperlimit1` *val*The requirements and meaning for this are the same as for the 
...
<a name="AEN87760"></a>## EXAMPLES

<a name="AEN87762"></a>**Example 1. Sample use of mqdc command for timestamping with external oscillator**
...
[[r87267#mqdccreate]]                This command creates an object to manage an MQDC-32
              ...
              [[r87267#mqdcmakethresh]]                This highlights the fact that the configuration file is
                ...
                some external file.
              [[r87267#mqdcthresholds]]                This command uses the `thresholds` variable
                and programs the channel thresholds of the QDC.

<a name="AEN87784"></a>**Example 2. Sample configuration of mqdc for event counting and interrupt readout**
