...
# <a name="daq1.mg_readout_wizard"></a>mg_readout_wizard

<a name="AEN20165"></a>## Name

mg_readout_wizard -- Create managed readouts

<a name="AEN20168"></a>## Synopsis

**$DAQBIN/mg_readout_wizard *database-file*
**

<a name="AEN20172"></a>## DESCRIPTION
...
XIA/DDASThis is the split XIA Readout/hit sorting readout system
                    that was introduced in NSCLDAQ-11.4 and is in NSCLDAQ-12.0
                    and later.

XXUSBThis is a readout program for either the VMUSB or CCUSB
                    smart crate controllers.

CustomThis is any readout program that can can load the
...
The Common paneThe top pane prompts for information that is common to all
                    readout types.

Type specific paneThe contents of this pane vary dependingon the type of
                    readout program selected (from the common pane).  It prompts
                    for information needed by the specific readout program type
                    you are using.

Action paneProvides the buttons needed to decide what to do with the
...
<a name="AEN20216"></a>### The Common Pane

The common pane provides the following inputs:



Readout NameThis is a name which will be used to derive the names
...
Program typeThis is a radiobutton set that selects the readout program
...
ContainerThe container part of the form allows you to select the
...
DirectorySets the current working directory for all of the programs.
                        Note that if the programs are run in the containerized
                        environment, this path must be valid within that
                        environment.

Source IdSelects the source Id that will tag the events produced
                        by the readout. The source id of each data source must be
                        unique for the event builder to function properly.

Output RingName of the ring buffer that will receive events from
                        the readout.

REST Service nameProvides the name of the REST service you want the
...
<a name="AEN20258"></a>### XIA specific pane
...
Sort HostThe system in which the hit sorting program will run.
...
Sort output ringDetermines the name of the ring buffer in which they
                        hit sorter will write the sorted hits.

Sort WindowDetermines the number of seconds in the sliding
                        window within which hits can be emitted.

Readout ThresholdThe number of words that must be in the FIFO of modules
                        in order to trigger a readout.

BuffersizeThe size of the ring item the readout will use to
                        send hit aggregates o the sorter.

Clock parametersIf Infinity is checked  the
...
ScalersSpecifies the number of seconds between scaler readouts.
                        This should not be smaller than 2 as we've observed
                        issues with the modules if it is.

<a name="AEN20294"></a>### XXUSB specific pane
...
Device SelectionIf the By serial checkbutton is checked,
...
DAQConfigurationProvides for the daqconfig file, which defines the
...
Control configurationIf the Use Control Server
...
ProgramThe path to the program that will perform the readout
                        (this path must be valid in whatever container environment
                        you might have selected).

Program optionsThese are option/value pairs which are passed to the
                        program on the command line, prior to any parameters.

Program parametersThese are an ordered list of parameters that are
                        passed on the command line after the options.

EnvironmentThese are environment variable name/value pairs
...
`--sourceid`Sets the source id that events should be tagged with.

`--ring`Specifies the ring buffer into which data will be
                        written.

--init-scriptThis will have a value that specifies the initialization
...
<a name="AEN20361"></a>## Database Objects created
...
xxx_readoutThe actual readout program itself.

xxx_initRequests a hardware initialization from the Readout.

xxx_setrunReads the run number from the database KV store and
                    asks the Readout program to set its run number
                    to match.

xxx_settitleReadss the run title from the database KV store and asks
                    the readout program to set its title to match.

xxx_beginrunAsks the readout program to start data taking.

xxx_endrunAsks the readout program to end data taking.

xxx_shutdownAsks the readout program to exit.

The following sequences are created and maintained:



bootreadoutsTriggered by the BOOT transition.
                    This will have xxx_readout
                    appended to it.

initreadoutsTriggered by the HWINIT transition.
                    This will have xxx_init appended to it.

beginreadoutsTriggered by the BEGIN transition.
                    In order,
                    xxx_setrun, xxx_settitle and
                    xxx_beginrun are appended to this
                    sequence.

endreadoutsTriggered by the END transition.
                    xxx_endrun is appended to this sequence.

shutdownreadoutsTriggered by the SHUTDOWN transition.
...
| --- | --- | --- |
| Prev | Home | Next |
| mg_container_wizard | Up | mg_logwizard |
