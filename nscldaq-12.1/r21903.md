...
<a name="AEN21917"></a>## Name

dumper -- Produce a formatted dump of event data.

<a name="AEN21920"></a>## Synopsis

**dumper *?options?*
**

<a name="AEN21924"></a>## DESCRIPTION
...
`--count`=*int*Requests that the program exit after it has printed
                    *int* items.

`--exclude`=*type-list*Requests that any items in the `type-list`
...
`--help`Provides a quick help on the program options.

`--skip`=*int*Skips *int* items before starting
...
`--sample`=*type-list*Requests that the item types in `type-list`
...
`--scaler-width`=*bits*Specifies the width of scaler counters in bits.  Unused
...
`--source`=*URL*Specifies the data source.  If omitted, the ring buffer named
...
`--nscldaq-version` *10 | 11 | 12*Specifies the version of NSCLDAQ the event file was written in.  
                    Defaults to 12

`--detail` *headers | bodies | fragments*Controls the amount of information dumper displays for each ring item in the
                    source.  headers displays only a brief header for each item.
                    bodies, the default, displays the formatted ring item bodies.
                    This is the information that dumper displayed prior to NSCLDAQ-12.1.
                    fragments  breaks out the fragments for event built data
                    for physics events.

The use of `--detail`=fragments on non-event-built
                    data is undefined and likely to make dumper crash.
