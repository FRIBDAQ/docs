...
# <a name="AEN22955"></a>SoftwareTrigger

<a name="AEN22959"></a>## Name

SoftwareTrigger -- Classify events for later filtering in the software trigger.

<a name="AEN22962"></a>## Synopsis

**$DAQBIN/SoftwareTrigger [OPTIONS]...
            **

<a name="AEN22966"></a>## DESCRIPTION
...
<a name="AEN22971"></a>## OPTIONS

Most options are mandatory.  Optional options are indicated.



`--help` (optional)Prints brief program help text to stdout.  After printing
                        the help text, the program exits.

`--help` (optional)Prints out the program's version to stdout. Once the
                        version is output, the program exits.

`--source`=URIA file or tcp URI that specifies where data will come from.
...
`--sink`=URIA file or tcp URI that specifies where the output
                        data will be written. Note that if this is a
                        tcp URI, the host must be local else an error
                        will abort the program.

`--workers`=integer (optional)Specifies the number of parallel workers to be used in threaded
...
`--clump-size`=integerSpecifies the number of ring items that make up a work unit
...
`--classifier`=filenameProvides the path to the shared object that will be loaded.
                        See CLASSIFIER LIBRARY below for more
                        information on what that library must contain.

`--parallel-strategy` = threaded | mpiSpecifies the parallelization strategy/communications
...
<a name="AEN23032"></a>## CLASSIFIER LIBRARY
...
<a name="AEN23040"></a>> ` virtual  uint32_t operator()(CRingItem& item);`
...
| --- | --- | --- |
| Prev | Home | Next |
| ringdiagnostics | Up | EventFilter |
