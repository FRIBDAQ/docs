...
<a name="AEN103475"></a>## Name

ReadoutUI -- Combined Readout Control Panel (REST VIEW).

<a name="AEN103478"></a>## Synopsis
...
<a name="AEN103480"></a>## DESCRIPTION
...
`-title`Sets/gets the readonly title label on the paramters
                     part of the view.

`-nexttitle`Sets/gets the contents of the entry field that is to
                     hold the title of the next run in the parameters part of
                     the view.

`-run`Set/gets the contents of the readonly run number label on
                     the parameters part of the view.

`-nextrun`Sets/gets/ the contents of the spinbox that is to hold the
                     next run number in the parameters part of the view.

`-titlecommand`Script that will be invoked when the next title entry has been
...
`-currentstate`Sets the current state of the run in the status/state
...
`-statecommand`Sets a script to be 
...
`-statistics`Passes Readout statistics to the
