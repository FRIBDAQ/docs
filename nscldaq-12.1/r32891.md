...
uint32_t` s_size`The size in bytes of the entire ring item.  The
                                size includes the size of the RingItemHeader
                                as well.

uint32_t`s_type`Contains the ring item type.  See the section
...
uint32_t`s_size`Contains the size of the ring item header.  note that
...
uint64_t`s_timestamp`The timestamp of the event in ticks of the clock
                                that defines event building.

uint32_t`s_sourceId`The id of the source that produced this item.
...
uint32_t`s_barrier; `If nonzero, this item is part of a barrier
...
#define TITLE_MAXSIZE 80Defines the maximum number of characters in a
