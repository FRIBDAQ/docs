...
`-irqeventthreshold` *0-32767*(From MDPP-16 QDC documentation) Every time the number of events in the FIFO
...
`-irqdatathreshold` *0-32256*(From MDPP-16 QDC documentation) Every time the number of 32 bit words in
...
`-maxtransfer` *0-32256*(From MDPP-16 QDC documentation) With
...
Bit[1:0](From MDPP-16 QDC documentation)
...
`-windowstart` *int 0-32767 (0-0x7fff)*(From MDPP-16 documentation) Unit: 1.56 ns (=25ns/16)
...
`-windowwidth` *int 0-16383 (0-0x3fff)*(From MDPP-16 documentation) Unit: 1.56 ns, max 16k = 25.56 us
...
`-pulseramplitude` *int 0-4095 (0-0xfff)*(From MDPP-16 documentation) Max value corresponds to about 30% at gain=1.
...
`-triggersource` *int 0-1023 (0-0x3ff)*(From MDPP-16 documentation) Defines the trigger which creates the window of interest.
...
`-triggersource` *int 0-1023 (0-0x3ff)*(From MDPP-16 documentation) Defines the trigger which creates output trigger.
                             This can be: one or both of the trigger inputs, lower 16 channels(B0), upper 16 channels(B1),
                             or both banks (all channels).
...
`-gaincorrectionshort`
*int[8] none | div4 | mult4*(From MDPP-16 documentation) The internal gains and hardware gain are
                             calculated based on the signal width and amplitude. This should give a quite
                             good start value. Detais of the signals will have an effect on the real
                             amplitude. So, there are 2 scaling factors to correct the gains.

`-nooffsetcorrection`
*bool[8] true | false*(From MDPP-16 documentation) The 4 samples before transmitted samples are used
...
`-noresampling`
*bool[8] true | false*(From MDPP-16 documentation) Usually the input signals are asynchronous to the
