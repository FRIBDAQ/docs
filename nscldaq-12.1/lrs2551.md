...
`-cumulative` *on|off*If `-cumulative` is false (default),
...
`-id` *vsn*Supplies the virtual slot number; *vsn*
...
`-insertid` *true|false*If true (the default is false),
...
`-slot` *slot*Sets the `slot` configuration.
