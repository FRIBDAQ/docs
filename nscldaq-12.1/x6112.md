...
| Prev | Chapter 9. Event Builder | Next |


---

# <a name="AEN6112"></a>9.2. Using the event builder
...
When the "Start" button is pushed in the ReadoutGUI, this will add a set
      ...
      details about the callouts in the [[c15873]] as well
      in the [[r78831]].
...
The next thing that you need to do is register clients
      ...
      of how to create one at the [[x15780]].
...
Some items in NSCLDAQ don't have body headers.  The
      `defaultid` parameter allows you to define the
      source id that will be attached to those parameters.   The default
      for this is the first id in the `ids` list of valid
      source ids.  The `defaultid` *must*
      be one of the ids in `ids`.
