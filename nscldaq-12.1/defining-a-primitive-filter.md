...
| Prev | Chapter 78. Filter framework | Next |


---

# <a name="AEN16485"></a>78.3. Defining a primitive filter
...
Finally as of NSCLDAQ 12.0, it is possible for a filter handler to
      emit more than one ring item. The `CFilterMain`
      has a method `putRingItem` that accepts a
      pointer to a ring item object and emits that item to the data sink.
      If you instantiate your filter to contain a pointer or reference
      to your `CFilterMain` application object, your
      filter elements have access to this method and can emit
      ring items in addition to returning a ring item to be emitted
      by the framework.
...
| --- | --- | --- |
| Prev | Home | Next |
| Getting Started | Up | Building a composite filter |
