...
# <a name="AEN911"></a>CDBEventWriter

<a name="AEN915"></a>## Name

CDBEventWriter -- SpecTcl event writer.

<a name="AEN918"></a>## Synopsis

```
CDBEventWriter
```

<a name="AEN1025"></a>## DESCRIPTION
...
`CTCLException`The save set and configuration save
...
`std::logic_error`All direct sqlite operations are
                                        statuc checked and failures result
                                        in this exception.

std::invalid_argumentIf the `pStateTransition`
...
`CTCLException`The spectra that are autosaved
...
std::logic_errorThrown for any sqlite3 error.

std::invalid_argumentIf `pScaler`
                                        does not point to a
                                        PERIODIC_SCALERS
                                        ring item.
...
int`s_runNumber`The run number represented by the record.

std::string `s_config`The name of the save set under which this
                            run data are saved.

std::string `s_title`Title of the run.

time_t `s_start`Timestamp describing the clock time at which
                            the run started.

time_t s_endTime at which the run ended.  This will be
                            undefined if `s_hasend`
                            is false

bool `s_hasEnd`True if the end time of this run is not null.
