...
The `SpecTcl` class provides an application programming
        ...
        (described in [[*The SpecTcl event sink pipeline.*|c1223]]),
...
[[*The API and the parameter dictionary*|c752#sec.api.parameters]]The API provides methods to inspect and maniuplate the
...
[[*The API and the spectrum dictionary*|x867]]The API provides methods that allow you to query and
                    manipulate the SpecTcl spectrum dictionary.    These
                    methods include methods that can create spectra of any
                    type.

[[*The API and the gates dictionary*|x990]]The API provides methods that allow you to query, define
                    and apply gates.

[[*The API and the event processing pipeline*|x1105]]The API provides methods to query and manipulate the event
                    processing pipeline.

[[*The API and the event sink pipeline*|x1176]]The API provides methods to query and manipulate the event sink
...
[[*The API and SpecTcl Objects.*|x1194]]The API provides methods to gain access to several important
...
[[![(1)](../images/callouts/1.gif)|c752#api.params.inchist]]                    The `CHistogrammer` class is a repository for
...
[[![(2)](../images/callouts/2.gif)|c752#api.params.incparams]]                    The parameter dictionary contains `CParameter`
...
[[![(5)](../images/callouts/5.gif)|c752#api.params.iterefirst]]                    The contents of a parameter dictionary are
