...
[[*The SpecTcl processing model.*|c68]]Which covers the processing model used by SpecTcl.  This will
...
[[*The SpecTcl event processing pipeline.*|c89]]The event processing pipeline is the set of objects that
...
[[*The SpecTcl event sink pipeline.*|c1534]]The event sink pipeline takes over where the event
...
[[*libTcl++ and adding commands to SpecTcl*|c1617]]SpecTcl uses the Tcl scripting language as its base command
...
[[*Extending SpecTcl spectrum file formats*|c1831]]The SpecTcl **sread** and **swrite**
...
[[*Extending SpecTcl's filter file formats*|c2075]]The format of files written by SpecTcl filters is extensible.
                    In this chapter we'll show how to extend the set of formats
                    available.

[[*Extending the set of SpecTcl fit types.*|c2377]]The SpecTcl fitting subsystem allows you to add additional
                    fit types.  In this chapter we'll show how to add a new
                    fit-type to SpecTcl.

[[*Interfacing SpecTcl with data from other data acquisition systems.*|c2819]]SpecTcl is data acquisition system neutral.  This chapter
...
[[*Dynamic processing pipelines*|c4354]]Describes new features for SpecTcl 5.1 which allow you to
