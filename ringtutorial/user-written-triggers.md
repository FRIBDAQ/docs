...
`setup`Called as the run is starting.  This allows the trigger
...
`teardown`Called as a run is halting (pausing as well as ending).
...
`operator()`This is called when the framework is able to accept a trigger.
...
| --- | --- | --- |
| Prev | Home | Next |
| Format of Ring bufffer DAQ event files |  | Compatibility mode utilities |
