...
These files are described in the documentation of [[dir_936f9f2cb541c58938765a02eba41f87]] . This section will summarize the files needed for nscope. By convention a directory subtree is used to hold the configuration files for each crate. The tree looks like this:
...
- `cfgPixie16.txt` must be modified to reflect the layout of modules in your crate and the actual desired name of the parameter settings file. The format of the file is described in detail in [[The_01cfgPixie16_8txt_01file#cfgPixie_format]]. `nscope` and `Readout` both use this file.
...
Note that the nscope program controls many more parameters of the DPP algorithm. See [[nscope]] for more information about what the nscope program can do.
...
- Set the FD Thres. (ADC) to be the LED threshold required to trigger the channel. I'm going to use 1000. In general this signal should be below the [[classCFD]] fraction of the smallest waveform you will take, because the [[classCFD]] zero crossing must occur after the LED is satisfied.


Apply the settings. Finally enable the [[classCFD]] by selecting `UV_Setup->CSRA` and setting the `CF` bit in all of the channels for which you are going to use the [[classCFD]].
...
For full documentation of the Readout program, see [[dir_936f9f2cb541c58938765a02eba41f87]]
...
For more information about the format of each of these see [[singlecrate#dataformat]]
...
This section describes the bit fields present in words of a DDAS event. This information is supplemental to the format information at: [[singlecrate#ddas1_ss1_dumper]]
...
NoteThe interpretation of the [[classCFD]] bits above pertain specifically to the 100 MSPS Pixie-16 digitizers at the NSCL. The meanings of the bits changes slightly when working with the 250 MSPS and 500 MSPS digitizers.
