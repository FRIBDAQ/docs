[← NSCL DDAS: Multi-crate System Setup Tutorial](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/simple_setup_multicrate.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [NSCL DDAS: Member List →](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/structCMyEndCommand_1_1EndEvent-members.md)
...
- [Introduction](#singlecrate_intro)
- [Overview of the components of a DDAS data taking system.](#singlecrate_dataflow)
- [Loading the Plx 9054 Kernel Module](#ddas_ss1_loadModule_sec)
- [Configuration files](#ddas_ss1_config)
- [Setting up the hardware](#ddas_ss1_hardware_setup)
- [Using Nscope to manage the DPP parameters](#ddas_ss1_nscope)- [Setting up the pulser signal shape](#ddas_ss1_pulsersetup)
  - [Setting up the module](#ddas_ss1_configmodule)
  - [Setting up the cfgPixie16.txt config file.](#ddas_ss1_configfiles)
- [Using DDAS Readout](#ddas_ss1_readout)- [Running Readout](#ddas_ss1_runningreadout)
  - [Environment variables](#ddas_readout_env)
  - [Starting the NSCLDAQ dumper program.](#ddas1_ss1_dumper)
  - [Taking data and looking at the format:](#ddas_ss1_dataformat)
- [Running Readout from the ReadoutGUI](#ddas_ss1_readoutgui)- [Setting up ssh keys for password-less login](#ddas_ss1_sshkeys)
  - [Stage areas and recording data](#ddas_ss1_stagearea)
  - [Setting up the data sources](#ddas_ss1_dsourcesetup)
  - [Setting up the event builder](#ddas_ss1_eventbuilder)
...
These files are described in the documentation of [readout](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/dir_936f9f2cb541c58938765a02eba41f87.md) . This section will summarize the files needed for nscope. By convention a directory subtree is used to hold the configuration files for each crate. The tree looks like this:
...
The directory `$DDAS_SHARE/readout/crate_1` has sample configuration files. Note, as of nscldaq 11.3, DDAS Readout and its examples are moved into NSCLDAQ. file described above is in `$DAQROOT/share/ddasreadout/crate_1` DDAS readout itself is in `$DAQBIN/DDASReadout` if an external clock is used for timestamp, this file is `$DAQBIN/DDASReadout_extcl`
...
Once you've created `~/readout/crate_1` finish making the structure below by
...
- One of the Pixie-16 modules must be setup as the clock master. This card must be inserted in slot 2 of the crate. Exactly one clock master must be configured or else very likely the host system will fail spectacularly.
...
2. Invoking the command `$DDAS_BIN/nscope`


The terminal window in which you run `nscope` will output a bunch of stuff. The main bits you want to look at are the crate map that is printed out. This will look something like
...
![nscope-startup.jpg](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/nscope-startup.jpg)
nscope start screen
...
- Boot the digitizers.
- Set the waveform signal conditioning parameters to make your signal acceptable for digital signal processing.
- Set the trigger conditions for the module.
- Save the DPP parameters into a .set file


Note that the nscope program controls many more parameters of the DPP algorithm. See [nscope](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/nscope.md) for more information about what the nscope program can do.


## Setting up the pulser signal shape


Set up your pulser to output pulses that rise quickly but have about a 50μsec fall time. The pulse polarity should be – with an amplitude no larger than 1V. For our test we'll use a 0.5V signal with a rise time of – and a fall time of 50μsec.


## Setting up the module


When using several DDAS cards it is important to synchronize the digitizer clocks between them. This is done by setting the jumper block JP01 appropriately. The jumper block is just a set of pins located on the board near the backplane connectors. Configure the connections between these pins with jumpers. In case your setup consists of a single module, be sure to configure the jumpers to match the top diagram for PXI clock master.
...
![ss1crate_jumpers.png](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/ss1crate_jumpers.png)
The jumper settings for a single crate system.


In this mode, the digitizer drives the clock on the PXI backplane with its own internal clock, and then reads it in from the backplane.
...
- Use the bottom jumper configuration for all modules to the right. Furthermore in this jumper configuration, all modules must be contiguous in the crate (no empty slots between modules).


## Setting up the cfgPixie16.txt config file.


Our simple setup uses a single digitizer module in slot 2 of the PXI crate. We are also going to use the DPP settings file crate_1.set This means our `cfgPixie16.txt` file will look like this:
...
- The first line of the file documents the crate number in use (Crate 1). `nscope` ignores this line as it is an inherently single crate system.
- The second line says we have only one digitizer module.
- The third line says this digitizer is in slot 2. If you have additional digitizers, the second line must be the number of digitizers you are using in this crate and there must be a line of slot numbers for each digitizer you are using.
- The final line says that `nscope` will load the settings contained in the file `crate_1.set` to configure the Pixie-16 modules during the Boot sequence.


Let's walk through the minimum steps required in nscope to set up the readout properly.


First click the `Boot` button at the left of the user interface. This
...
If the boot fails with an error, more detailed error messages will be written to `Pixie16msg.txt`


In order to set the waveform conditioning parameters, you must look at a waveform. Click the `Read` `WF` button. If the digitizer does not capture a full waveform, click it again until one is captured.
...
- Set the FD Thres. (ADC) to be the LED threshold required to trigger the channel. I'm going to use 1000. In general this signal should be below the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/classCFD.md) fraction of the smallest waveform you will take, because the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/classCFD.md) zero crossing must occur after the LED is satisfied.


Apply the settings. Finally enable the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/classCFD.md) by selecting `UV_Setup->CSRA` and setting the `CF` bit in all of the channels for which you are going to use the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/classCFD.md).
...
Once all channels are set up, save the settings using the `UV_Setup->Save2File` menu command. In the file selection dialog that pops up, choose the .set file you specified in your `cfgPixie16.txt` file. This file will be loaded into the modules automatically when they are booted next.


# Using DDAS Readout


Once you have a good settings file, you can take data with Readout. In this section we will:


- Run the DDAS Readout program.
- Run an nscldaq dumper program.
- Take a little data and describe the format of raw DDAS data read by Readout.


For full documentation of the Readout program, see [readout](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/dir_936f9f2cb541c58938765a02eba41f87.md)


## Running Readout


The DDAS readout program does not need to be modified by you. It uses a set of configuration files that are expected to live in the current working directory when Readout is run:


The directory `$DDAS_SHARE/readout/crate_1` has sample configuration files. If you are using NSCLDAQ/11.3 or higher, this example has been moved to `$DAQBIN/share/ddasreadout/crate_1`
...
Note that if you are using NSCLDAQ-11.3 or higher run `$DAQBIN/DDASReadout` or, for externall clock; `$DAQBIN/DDASReadout_extcl`
...
## Starting the NSCLDAQ dumper program.


When you ran the `ddassetup.bash` script to incorporate the DDAS environment variable in your shell, that script also ran the daqsetup.bash script for the version of NSCLDAQ against which the DDAS software was built.


To run the dumper, therefore, you only need to open a new terminal window and type:
...
The following longword is the low order 32 bits of the 48-bit event timestamp. Note that while the timestamp in the event body is a raw digitizer timestamp, the timestamp put in the body header (labelled by the text "Timestamp:" in the dump above is in nanoseconds. This allows the event builder to compare timestamps between a heterogeneous set of digitizers in the system.


For more information about the format of each of these see [Appendix - data format](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/singlecrate.md#dataformat)
...
In addition to running and commanding the DDAS Readout program, ReadoutGUI can initiate data recording. To do this, ReadoutGUI must be told where to record data. ReadoutGUI expects a symbolic link in your home directory named stagearea. This link points to a directory structure into which event data are logged. For more information about the directory structure ReadoutGUI maintains, see the NSCLDAQ user guide section on the ReadoutGUI.


[Click here for a link to the user's guide](http://docs.nscl.msu.edu/daq)
...
3. In the resulting dialog, Fill in the fields as follows:
...
Click the `Start` button to start your data source (Readout).
...
This section describes the bit fields present in words of a DDAS event. This information is supplemental to the format information at: [Starting the NSCLDAQ dumper program.](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/singlecrate.md#ddas1_ss1_dumper)
...
- The slot identifier ???
- The header length is the number of longwords prior to any trace data. This is self inclusive.
- Event length is the total number of longwords in the event. Since we are not capturing trace data, this is the same as the event header length.
- The most significant bit is 0 if the event was a good event and 1 if the event was a pile-up event.


- Note
  The interpretation of the [CFD](https://github.com/FRIBDAQ/docs/tree/main/ddas-1.1/classCFD.md) bits above pertain specifically to the 100 MSPS Pixie-16 digitizers at the NSCL. The meanings of the bits changes slightly when working with the 250 MSPS and 500 MSPS digitizers.
