...
These files are described in the documentation of [[readout|dir_936f9f2cb541c58938765a02eba41f87]] . This section will summarize the files needed for nscope. By convention a directory subtree is used to hold the configuration files for each crate. The tree looks like this:
...
- `cfgPixie16.txt` must be modified to reflect the layout of modules in your crate and the actual desired name of the parameter settings file. The format of the file is described in detail in [[The cfgPixie16.txt format|The_01cfgPixie16_8txt_01file#cfgPixie_format]]. `nscope` and `Readout` both use this file.
...
Note that the nscope program controls many more parameters of the DPP algorithm. See [[nscope|nscope]] for more information about what the nscope program can do.
...
- Set the FD Thres. (ADC) to be the LED threshold required to trigger the channel. I'm going to use 1000. In general this signal should be below the [[CFD|classCFD]] fraction of the smallest waveform you will take, because the [[CFD|classCFD]] zero crossing must occur after the LED is satisfied.


Apply the settings. Finally enable the [[CFD|classCFD]] by selecting `UV_Setup->CSRA` and setting the `CF` bit in all of the channels for which you are going to use the [[CFD|classCFD]].
...
For full documentation of the Readout program, see [[readout|dir_936f9f2cb541c58938765a02eba41f87]]
...
For more information about the format of each of these see [[Appendix - data format|singlecrate#dataformat]]
...
This section describes the bit fields present in words of a DDAS event. This information is supplemental to the format information at: [[Starting the NSCLDAQ dumper program.|singlecrate#ddas1_ss1_dumper]]
