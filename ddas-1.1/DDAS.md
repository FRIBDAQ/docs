...
The data format is described in detail in the [[readout|dir_936f9f2cb541c58938765a02eba41f87]] section. However, the interpretation of the [[CFD|classCFD]] time is unique to each module. The 3rd data word in the event header contains the [[CFD|classCFD]] time in its most significant 16 bits. The bits have the following meaning:
...
The table above shows that the timestamp in the module has a factor of two difference between the frequencies it timestamps with and it digitized with. This is really just a difference between the frequencies of the FPGA and ADC. This factor of two causes some interesting features in the [[CFD|classCFD]] time that the module outputs. The [[CFD|classCFD]] time provides the correction for the fractional offset from the most recent sample point and also which sample point to adjust from (there are two samples per each tick on the timestamp clock). The meanings of the bits in the third 32-bit header word are as follow:
...
To compute the [[CFD|classCFD]] correction to the timestamp from this, one would do the following computation:
...
Like the 250 MSPS modules, there is a discepancy between the timestamp and ADC frequencies. The [[CFD|classCFD]] time therefore also accounts for this detail. The third 32-bit header word emitted by the Pixie-500 module has the following bit meanings:
...
The trigger distribution board is used to distribute the clock and triggers between crates in a multicrate system. See the [[The Clock/Trigger Distribution Board|simple_setup_multicrate#ssncrate_hdwrConfigClockDist_sec]] for a description of this device and how to use it.
