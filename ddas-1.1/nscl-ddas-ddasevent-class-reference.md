...
Any data that was written to disk downstream of the NSCLDAQ event builder will have a "built" structure. What that means is that the body of the physics event item will contain data from more than one DDAS event. The [[classDDASEvent]] class represents this type of data. It provides access to the events that make it up through the ddaschannel objects it owns and then also provides some useful methods for getting data from the event as a whole.
...
The documentation for this class was generated from the following files:- ddasdumper/[[DDASEvent_8h_source]]
