...
- [[classDDAS_1_1SettingsReader]] is the base class of a family of classes that read DSP settings for a single module.
- [[classDDAS_1_1CrateReader]] is the base class of a family of classes that read DSP Parameters for a collection of modules. Normally, these modules live in a single PXI crate.
...
- [[classDDAS_1_1SettingsWriter]] is the base class of a family of classes that write DSP parameters for a single module.
- [[classDDAS_1_1CrateWriter]] is the base class of a family of classes that write DSP parameters for a crate full of modules.


## Readers


Readers take DSP parameters from some source and store them in an internal representation. The internal representations used are structs that are defined in the [[ModuleSettings_8h]] header. There are two types of Readers. Readers that take settings from a single module worth of stuff and Readers that take settings from a Crate worth of stuff. The abstract base classes for both of these use the strategy pattern to provde the main logic in the base class while providing source dependent code in derived, concrete classes.
...
Let's have a look at the base class, [[classDDAS_1_1SettingsReader]] described in the header [[SettingsReader_8h]] :
...
- [[classDDAS_1_1PixieSettingsReader]] which reads DSP settings from one Pixie16 module.
- [[classDDAS_1_1SetFileReader]] which reads DSP settings from one module in a .set File.
- DDAS::XmlSettingsReader which reads data from XML files see [[libDSPParameters#dsp_xml]]
...
Crate readers are intended to read a collection of modules. The [[classDDAS_1_1CrateReader]] base class uses a strategy pattern to provide the main logic flow for reading a crate allowing the concrete subclasses to provide readers for individual modules. Here's the definition of [[classDDAS_1_1CrateReader]] from the header [[CrateReader_8h]]:
...
Only public and protected members are shown. m_slots is a vector of slot numbers that must be rea and m_crateId is the crate id to be given to the collection. This crate id overrides the crate id from the modules themselves. The pure virtual method: createReader must produce a [[classDDAS_1_1SettingsReader]] for the slot number provided. Note this is a slot number not a module id.


Concrete subclasses include:


- [[classDDAS_1_1PixieCrateReader]] (defined in [[PixieCrateReader_8h_source]]) which reads data from a PXI crate.
- [[classDDAS_1_1SetFileCrateReader]] (defined in SetFileCratereader.h) which reads data from a Set file.
- [[classDDAS_1_1XMLCrateReader]] (defined in [[XMLCrateReader_8h]]) which reads data from an XML file. See [[libDSPParameters#dsp_xml]] for information about the format of these XML files.


## Writers


The concept of Writers mirrors that of Readers. There is the [[classDDAS_1_1SettingsWriter]] class which writes a module's worth of data and [[classDDAS_1_1CrateWriter]] which writes a PXI crate worth of data.


Here's the class definition of [[classDDAS_1_1SettingsWriter]] from [[SettingsWriter_8h]]
...
Constructing a concrete [[classDDAS_1_1SettingsWriter]] is supposed to connect it with its sink. The write method then writes [[structDDAS_1_1ModuleSettings]] to that module. This method is pure virtual and must be implemented in concrete classes. These include:


- DDAS::PIxieCrateWriter ([[PixieCrateWriter_8h]]) which writes to a module in a PXI crate.
- [[classDDAS_1_1XMLSettingsWriter]] ([[XMLSettingsWriter_8h]]) which writes to an XML file whose format is described in [[libDSPParameters#dsp_xml]]
- [[classDDAS_1_1SetFileWriter]] ([[SetFileWriter_8h]]) which writes XIA bulk settings files.


Similarly the [[classDDAS_1_1CrateWriter]] writes a collection of modules in a PXI crate. Modules can be selectively loaded. Here's the definition of [[classDDAS_1_1CrateWriter]] from [[CrateWriter_8h_source]]:
...
- getWriter provides a [[classDDAS_1_1SettingsWriter]] for the specified slot number (not Module). The caller will delete this object so it must be dynamically instantiated.


Currently the following concrete [[classDDAS_1_1CrateWriter]] subclasses have been supplied:


- [[classDDAS_1_1PixieCrateWriter]] ([[PixieCrateWriter_8h]]) writes to a physical crate full of Pixie16 modules.
- [[classDDAS_1_1XMLCrateWriter]] writes a set of XML files whose format is described in [[libDSPParameters#dsp_xml]]
- [[classDDAS_1_1SetFileCrateWriter]] writes a set file that can be loaded using e.g. the XIA APi function Pixie16LoadDSPParametersFromFile (or a boot with bit 4 set in the BootPattern argument)
...
The header [[SetFile_8h]] provides definitions that allow you to read set files. These are presented as a static methods for a class in the DDAS namespace. A set file is essentially a soup of uint32_t values (with one exception). The header defines this class:
...
- The XIA header [[pixie16app__defs_8h_source]] provides the constant definition N_DSP_PAR. That's the number of uint32_t parameters that make up each module.
...
- The [[classDDAS_1_1CrateManager]] class which we will document next provides a static method to return the path to the variable file given the speed of a module.
- [[classDDAS_1_1SetFile#a4124c0d8fdc3693584506bf628acb9ee]] returns the size of the file and a pointer to the first of the uint32_t's in the file. The storage must be freed by calling [[classDDAS_1_1SetFile#ac67d5cdd6c27e261b855ce234210af4d]] when you are done with it.
- The PreampTau DSP Parameter is actually an IEEE float32 not a uint32_t simply moving the bits in this value into a float will do that conversion. See the code that extracts these values in [[classDDAS_1_1SetFileReader]]
- Saving the best for last, you really could just use SetFileReder to populate a [[structDDAS_1_1ModuleSettings]] struct (see [[ModuleSettings_8h]]) rather than call this set of functions directly.
...
The [[classDDAS_1_1CrateManager]] class provides a class that can manage a PXI crate that has Pixie16 digitizers. Here are the important featurs of that class (defined in [[CrateManager_8h_source]]). See the class documentation for more:
