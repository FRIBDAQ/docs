...
I will layout the structure of the data in a tree structure where there is a top-level event structure called [[structMyParameters]] that holds the multiplicity information as well as all of the channel data objects as a flat array of energy time pairs.
...
struct [[structMyParameters]] {





[[structChannelData]]          chan[48];
...
[[structMyParameters]](std::string name);
...
class [[classMyParameterMapper]] : public [[classDAQ_1_1DDAS_1_1CParameterMapper]]
...
[[structMyParameters]]& m_params;          // reference to the tree parameter structure
...
[[classMyParameterMapper]]([[structMyParameters]]& params);
...
int computeGlobalIndex(const [[classDAQ_1_1DDAS_1_1DDASHit]]& hit);
 ...
 fragment Note that there is not much to this class. The class maintains a reference to our parameters and an m_chanMap data member, which we will talk about a little later. This class is derived from the [[classDAQ_1_1DDAS_1_1CParameterMapper]] base class. Concrete [[classDAQ_1_1DDAS_1_1CParameterMapper]] classes are expected to provide a mapToParameters method that takes the hits and spectcl parameter and fills the parameters from the hits.
...
m_params.chan[globalChanIdx].energy    = hit.[[classDAQ_1_1DDAS_1_1DDASHit#a123d68755f678cd1eec6f5320a39b629]]();


m_params.chan[globalChanIdx].timestamp = hit.[[classDAQ_1_1DDAS_1_1DDASHit#a930e45be2c50eaf4a90988b0bd8c4a74]]();
...
MyParameterMapper::MyParameterMapper([[structMyParameters]]& params)
...
[[classDAQ_1_1DDAS_1_1CDDASBuiltUnpacker]] Stage1( {0, 1, 2 }, *(new [[classMyParameterMapper]](params)));

 fragment To round out our work on MySpecTclApp.cpp, you simply need to add some include directives to the top of the file. These will bring the [[structMyParameters]], [[classMyParameterMapper]], [[classDAQ_1_1DDAS_1_1CDDASBuiltUnpacker]] classes into scope. Add these lines to the top:
...
Add the [[structMyParameters]] and [[classMyParameterMapper]] class to the build by adding MyParameters.o and MyParameterMapper.o to the OBJECTS variable. It should look like this when you are done:


OBJECTS=MySpecTclApp.o [[structMyParameters]].o [[classMyParameterMapper]].o

 fragment Next the compiler needs to be told where to find the [[DDASBuiltUnpacker_8h_source]] file. Furthermore, the unpacker code uses C++ language and library elements from the 2011 version of the C++ standard so we need to enable the use of those by the compiler. That is done by adding to the USERCXXFLAGS so it looks like this:


USERCXXFLAGS=-[[namespacestd]]=c++11 -I/usr/opt/ddas/VERSION/include
