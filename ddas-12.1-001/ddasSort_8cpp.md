...
In order to get best performance from [[namespaceDDASReadout]], sorting of the resulting hits has been pushed downstream. This program reads ring items that consist of multiple hits from boards and outputs ring items that consist of single hits from all boards in an experiment sorted by time. Ring item bodies we take as input look like:
