...
`-cbltaddress` *base-address*Defines the base address to which the CBLT reads will be
...
`-mcastaddress` *base-address*Defines the base address for the chain's multicast address.
...
`-maxwordspermodule` *longword-coun*Defines the maximum words that can be read from each module.
...
`-modules` *module-name-list*Defines the list of MxDC32 modules that make up the chain.
