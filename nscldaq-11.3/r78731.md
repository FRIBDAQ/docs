---
---

[← FrameSequencer](https://github.com/FRIBDAQ/docs/tree/main/nscldaq-11.3/r78646.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [ScaleControl →](https://github.com/FRIBDAQ/docs/tree/main/nscldaq-11.3/r78793.md)

---

---

# <a name="tcl3_ringstatus"></a>RingStatus

<a name="AEN78745"></a>## Name

RingStatus -- Widget that shows ring status.

<a name="AEN78748"></a>## Synopsis

**
package require RingStatus
package require ring
            **

**
RingStatus *path ?options?*
**

** *path* configure *options...*
**

** *path* cget *option*
**

** *path* update [ringbuffer usage *ringname*]
            **

<a name="AEN78766"></a>## DESCRIPTION

Provides a Tcl megawidget that displays the status of a ringbuffer.
            The widget shows the name, size and total free space of the ring
            A table also shows the PIDs of each consumer along with the number of
            bytes of backlog data for that consumer.

<a name="AEN78769"></a>## OPTIONS



- `-name` *name-string*
  Configures the name of the ring. This is displayed in the widget.

<a name="AEN78778"></a>## METHODS



- `update` *ring-info-list*
  Accepts a list of data compatible with the output
                              of the
                              [**ring usage**](https://github.com/FRIBDAQ/docs/tree/main/nscldaq-11.3/r74988.md)
                              command.  This data is used to update the ring size, free space and
                              consumer information table.

<a name="AEN78789"></a>## SEE ALSO

[*TCL Ring package.*](https://github.com/FRIBDAQ/docs/tree/main/nscldaq-11.3/r74988.md)

---

---

[← FrameSequencer](https://github.com/FRIBDAQ/docs/tree/main/nscldaq-11.3/r78646.md) | [Home](https://github.com/FRIBDAQ/docs/tree/main/Home.md) | [ScaleControl →](https://github.com/FRIBDAQ/docs/tree/main/nscldaq-11.3/r78793.md)
