...
<a name="AEN64741"></a>## OPTIONS



`-trigger` *nim1 | scaler | interrupt*Defines the strigger source for the stack.  When the designated
...
`-period` *seconds*Defines the number of seconds (integer) between scaler stack triggers.
...
`-stack` *stacknum*Defines the stack number for interrupt
...
`-vector` *status-id*For interrupt `-trigger`-ed
...
`-ipl` *priority*Defines the interrupt priority level that of the interrupt
...
`-delay` *microseconds*The VM-USB allows you to define a delay between the stack trigger condition
...
`-modules` *module-list*Defines the set of modules that will be read out. This can be
...
<a name="AEN64851"></a>**Example 1. Building Stacks**
