...
# <a name="AEN51"></a>Chapter 2. The electronics
...
# <a name="AEN57"></a>2.1. A minimal electronics setup
...
<a name="AEN91"></a>**Figure 2-1. A simple electronics setup for the CAEN V785**
...
<a name="AEN97"></a>**Figure 2-2. Pulser Output Signal**

![](figures/ss785_pulser_output.png)

<a name="AEN100"></a>**Figure 2-3. Amplifer Output Signal**
...
*NOTE:* Ribbon cable can be difficult to
        work with because it is easy to get it twisted and lose track of
        which end is which. To avoid this look carefully at the coloring on
        the cable you are using and be sure it is plugged in the correct
        channel of the ADC. Also, the connectors typically have a marking on
        one end that conventionally specifies channel 0.

The other output from the splitter will go into a
        discriminator.  This could be either a leading edge or a constant
        fraction discriminator. The job of the discriminator is to ouptut a
        logic signal when an input signals has exceeded a certain voltage
        threshold. The output signal will be used to tell the ADC that a
        signal and thereby initiate the process of recording the peak height
        of the signal. The discriminator will have a threshold knob, be sure
        that the threshold is set above any background noise that may be
        present so that the discriminator only trips on the pulser signal.
        It will be useful to look at the both the logic pulse and the signal
        from the amplifier at the same time when setting the threshold
        value. Figure 1-4 shows what you will see.

<a name="AEN107"></a>**Figure 2-4. Amplified Signal with Logic Pulser**
...
<a name="AEN111"></a>**Figure 2-5. Amplified Signal and Gate**

![](figures/ss785_amplified_signal_with_gate.png)

Next we will take the "DRDY" output of the V785 and send that into
        IN0 of the CAEN V262. This signal will be used as the signal to
        trigger out readout.
