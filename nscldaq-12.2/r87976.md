...
<a name="AEN87990"></a>## Name

madc -- Acquire events from Mesytec MADC32 ADC.

<a name="AEN87993"></a>## Synopsis
...
`-gatemode` *mode*The MADC32 has a pair of gate inputs.  The inputs may
...
`-gategenerator` *onoff*The module can either use the gates as provided or can
...
on, true, yes, 1, enabledIf `-gatemode` is common,
...
off, false, no, 0, disabledNeither gate generator is enabled, regardless of
				`-gatemode`

gdg0Regardless of `-gatemode`, gate generator 0
				is enabled.  Gate generator 1 is not enabled.

gdg1If `-gatemode` is separate, gate generator
...
`-holddelays` *delayList*If the gate generators are enabled (see
...
`-holdwidths` *widthList*If the gate generators are enabled (see
                    `-gategenerator` above), the
...
`-inputrange` *rangeSelector*Programs the input range for the module.
...
`-ecltiming` *onoff*This parameter, when true enables the gate1 ECL input to
...
`-timingsource` *sourceName*Specifies the source of the clock for timestamps.
...
`-timingdivisor` *log2*Specifies a scale-down value for the timestamp clock.
...
`-nimbusy` *busyselecst*This option selects which signal is presented at the
...
busyThe default.  The output reflects the module
...
gate0The Gate0 signal.  Note that if the internal gate
...
gate1The Gate1 signal.  Again if the gate generator is
					enabled, you can use this to monitor the gate signal
					*after* it has been modified
					by the gate and delay generator.

cbusThe Busy output is used as a CBus master.  See
...
The default value for this parameter is
                      ...
                      single event mode. If separate gate mode is enabled, this 
                      parameter is set to true automatically.

`-maxtransfers` *integer*Set the maximum number of events to transfer before 
                    ending the transfer at the next end-of-event word in multievent 
                    mode.

This value is set automatically based on the gate mode and is 
                    generally not necessary for the user to configure directly.

`-irqthreshold` *integer*Sets the interrupt threshold.  When a number of
...
`-resolution` *2k|4k|4khires|8k|8khires*Sets the resolution of the module.  This has an impact on
                      the conversion time.

This defaults to 8k

<a name="AEN88301"></a>## EXAMPLES

<a name="AEN88303"></a>**Example 1. Sample use of madc command**
