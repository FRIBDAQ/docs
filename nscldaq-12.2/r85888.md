...
|  |  |
| --- | --- |
|  | Note that we have identified a problem when using these modules with a VMUSB
        and base addresses of the form0x00nn0000Until this can be resolved (if ever), configuring the module with base addresses
        like these (on the command line or using the-base) will result
        in an error processing the configurationfile. |

<a name="AEN85946"></a>## OPTIONS
...
**-inputs** *ribbon | nim*This is interpreted by SpecTcl 7.0-001 and later.  It allows it to 
                ...
                If one configures the module instead with -inputs nim
                SpecTcl 7.0-001 and later can handle tight packed parameter definitions.

If not specified this defaults to ribbon with the
...
**-supressunderthreshold** *bool*If false when **-supressrange** is
								 false, the module will only suppress under threshold
								 conversion. If not all channels over threshold will supply data for an
								 event. In most cases, this should be set to true.
								 This defaults to false because this is in effect
								 when **-supressrange** is set false.

When enabled, 13th bit (0x2000) of a data word will be set 1 if the data is
								 under threshold.

Make sure to set **-supressrange** to false
								 to make this option effective.

**-supressoverflow** *bool*If false when **-supressrange** is
								 false, the module will only suppress overflow
								 conversion. If not all channels over threshold will supply data for an
								 event. In most cases, this should be set to true.
								 This defaults to false because this is in effect
								 when **-supressrange** is set false.

When enabled, 12th bit (0x1000) of a data word will be set 1 if the data is
								 overflow.
...
<a name="AEN86110"></a>## EXAMPLES

<a name="AEN86112"></a>**Example 1. Sample ADC commands**
