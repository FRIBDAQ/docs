...
<a name="AEN88338"></a>## Name

mtdc -- Mesytec 32/34 channel TDC

<a name="AEN88341"></a>## Synopsis
...
<a name="AEN88364"></a>## OPTIONS



`-base` `base-address`Supplies the modules VME base address.
                  The `base-address`
                  value must match the base address as set in the
                  module rotary switches.

`-id` `module-id`Defines the module's id.  This value appears
...
`-ipl` `priority-level`Sets the interrupt priority level for interrupt
...
`-irqthreshold` `word-count`Determines the minimum number of 32 bit words the
...
The FIFO is 32K 32 bit words long so this
                  ...
                  0 and 0x7fff and
		  defaults to 1 which is suitable for single
		  event mode.

`-maxtransfers` word-countWhen the module is in multi-event mode 3, this sets
...
`-datalen` `length-spec`Determins how the event is padded for alignment.
                  while the values 8,
                  16, 32
...
`-multievent` `on |off |limited`Enables or disables multi-event mode.
...
`-skipberr` `on | off|`If true then at the end of the read rather
...
`-countevents` `yes | no`If true, then the `-maxtransfers`
                  value is taken to be the number of events rather
                  than the number of words.

By default this is false.

`-marktype` `timestamp | eventcount | extended-timestamp`Determines what the value in the event correlation
                  field of the data means.

By default, this is timestamp

`-joinedbanks` `yes |no`Only relevant in trigger matching mode when this determines if
                  the gates are ored in to both banks (yes
                  or if they affect individual banks
                  of 16 channels (no).  This defaults to
		  true.
...
`-resolution` `resolution-value`Sets the TDC resolution.  This is only relevant in
...
`-format` `standard |fulltime`Determines if the data are time differences from the
                  gate (trigger matching mode) or full timestamps
                  (timestamping mode).

Defaults to standard

`-bank0winstart` `ns`, `-bank1winstart` `ns`Determines when the matching window for trigger matching
...
`-bank0winwidth` `width`, `-bank1winwidth` `width`Determines the width of the trigger matching
                  window.  The `width`
...
`-bank0triggersource` `trigger-spec`, `-bank1triggersource` `trigger-spec`The actual triggering of the module is quite flexible.
...
Tr0The bank is triggered on the Gate 0 input.

Tr1The bank is triggered on the Gate 1 input.

Ch0 ... Ch31The specified channel is the trigger for the bank.

Bank0A hit in any channel of Bank 0 is the trigger for
                        the bank.

Bank1A hit in any channel of Bank 1 is a trigger
                        for the bank.

The default for `-bank0triggerrsource`
		    is Tr0, the trigger 0 input.  The default
		    for `-bank1triggersource` is
		    Tr1.

`-bank0firsthit` `boolean`, `-bank1firsthit` `boolean`If true, only the first hit from the bank
                  is
                  transmitted.  Otherwise all hits in the
                  bank's trigger matching window are
                  transmitted.

This defaults to false allowing multiple
		    hits from both banks of the module.

`-edge` `rising | falling`Determines which signal edge is timed.  This defaults to
		  falling.

`-tr0terminated` `boolean`, `-tr1terminated` `boolean`If this parameter is true, the ECL Trn input is
                  terminated by the module.  If not that
                  input is unterminated.

These parameters defaul to false which allow
		    the trigger inputs to be bussed to other ECL inputs.
		    The input at the end of the bus *must*
		    be terminated however.

`-resetterminated` `boolean`If true, the ECL reset input is terminated
                  otherwise it is unterminated.   This defaults to
		  false

`-ecltrig1isoscillator` `boolean`If true then the ECL trig1 input is treated
...
`-trigfromecl` `boolean`If true, the trigger comes from the ECL inputs,
                  otherwise it comes from the NIM inputs.  This
                  is an exclusive setting (see register
                  0x6068 documentation e.g.)

Defaults to false so that by default the
		    trigger inputs are the NIM inputs.

`-nimtrig1isoscillator` `boolean`The nim TRIG1 input can be used to supply
...
`-busy` `bothbanks | cbusoutput | abovethreshold`Describes the function of the busy output:



bothbanksThe busy is set when FIFO is full or
                        acquisition is halted (the module
                        cannot accept a busy).

cbusoutputThe busy output is used to run
                        a CBUS control bus.

abovethresholdThe busy output indicates the
...
`-pulseron` `boolean`If true, the test pulser is active.  This is
		  false by default.

`-pulserpattern` `bitmask`Sets the pulser pattern, which determines
                  which channels are
                  pulsed.  This is 0 by default.

`-bank0threshold` `level`, `-bank1threshold` `level`When the modue is jumpered for
...
This defaults to 105 which the manual
		    says is suitable for a NIM input.

`-timingsource` `vme | external`Determines the timing source for the
...
`-multhi0` `0-255`The largest number of channels that must
                  be
...
`-multlow1` `0-255`If banks are not joined the low multiplicity
                  required for an event from bank 1.  Defaults to
		  0

`-multhi1` `0-255`If banks are not joined, the high multiplicity
