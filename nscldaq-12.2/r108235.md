...
<a name="AEN108239"></a>## Name

ReadoutRESTClient -- Client for Readout REST servers

<a name="AEN108242"></a>## Synopsis
...
<a name="AEN108244"></a>## DESCRIPTION
...
`-host`THe host in which the Readout program we want to communicate
...
`-user`The user the Readout program you want to communicate with
...
`-service`The name of the servic advertised by the ReadoutREST
                    package in the Readout we want to communicate with.
                    This defaults to ReadoutREST which is
                    the default service name.

<a name="AEN108276"></a>## PUBLIC METHODS
...
`begin`Attempts to start a new run.  The resulting Readout
...
`end`Ends active data taking in the Readout program.  Returns
                    the name of the resulting state.

`init`Attempts to initialize the hardsware in the Readout.
...
`shutdown`Requests the Readout program to exit.

`getState`Returns the current Readout program state.

`getTitle`Returns the currently set title.

`getRunNumber`Returns the currently set run number.

`getStatistics`Returns a dict that describes the Readout's statistics.
...
triggersNumber of triggers sent to the Readout.

acceptedTriggersNumber of
...
bytesNumber of bytes read from the hardware.

`setTitle` `title-string`Sets the Readout's title to the specified
                    `title-string`

`setRunNumber5` `number`Sets the new run number to `number`.
...
| --- | --- | --- |
| Prev | Home | Next |
| loggerrestclient | Up | ReadoutStatistics |
