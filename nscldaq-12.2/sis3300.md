...
<a name="AEN86502"></a>## Name

sis3300 -- Simplified sis3300 support.

<a name="AEN86505"></a>## Synopsis
...
startIn start mode an external start input is required to start
...
stopIn stop mode, the digitizer is continuously digitizing.
...
gateIn gate mode, a gate signal is presented to
...
<a name="AEN86537"></a>## OPTIONS



`-base` *base-address*Sets the VME base address of the module.  For the 3300,
...
`-clock` *source-spec*Sets the digitizer clock source.  This can be one of
...
100Mhz, 50Mhz, 25Mhz, 12.5Mhz, 6.25Mhz or 3.125MhzSelects an internal clock source at the
				   specified frequency.  The default value is

externalThe clock is provided as an external symmetric
...
randomAllows arbitray clock trains or slow clocks.
...
hiraRuns in HiRa random clock mode.  This is suitable
...
`-mode` start | stop | gateSelects the mode of the digitizer.  Defaults to start mode.
...
`-startdelay` disabled | valueIf the value of this option is disabled,
...
`-stopdelay` disabled | valueSame as `-startdelay` above, however
			 the delay, if enabled, applies to the stop time.

Defaults to disabled

`-samples` *value*Selects the maximum number of sample that will be
...
`-thresholdslt` *list*A list of 8 booleans, one per digitizer channel.  If
			 a list element is
...
`-thresholds` *list*A list of 8 integer in the range [0,4096)
...
`-minletime` *list*This parameter is a list of four integers, one per channel
			 group that
...
`-mingttime` *list*Same as `-minletime` except that
...
`-trigwidth` *list*List of four integers in the range [0,16),
...
`-groupenables list`List of four booleans (defaults to all true),
...
`-header` *value*This 16 bit value is inserted into the data from the
...
headerIs the value of the `-header` option.  If not
		    configured by you this defaults to 0xfadc

trailerIs the value of the `-trailer` option

Group trigger event directorySee section 4.25 of the SIS3300/3301 manual for the
...
Size of data from groupThis is the actual number of samples that will appear
...
Data from the group.This is the raw sample data, 32 bits per sample containing
...
trailerThese 16 bits are the value of the `-trailer`
