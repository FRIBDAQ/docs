...
`-dt` ticksThe value of this option is passed as the
...
`-nobuild` boolIf the boolean value is true, then
...
`--timestamp-policy` valueThe value of this option is passed as the value of the
...
`-outsid` INTProvides the value of the source id for fragments emitted
                  by glom (glom's `--sourceid` option).  If not provided,
                  this defaults to 0.

`-maxfragments` INTProvides the largest number of fragments that can be grouped together
...
`-inbuffersize` INTProvides the evbtagger's `--buffersize` parameter.
...
`-resetts` BOOLIf true, the evbtagger program will reset its idea of the most
...
`-insid` INTProvides the default source id to use for input data.
...
`-inring` ringnameMandatory option that provides the name of the ring buffer
                  from which input event fragments are taken.  This ringbuffer
                  must already exist.  Note it is a name not a URL.

`-outring` ringnameMandatory option that provides the name of the output ringbuffer
...
`start`Starts the event builder pipeline. The event builder pipeline
...
`stop`All of the processes in the event builder pipeline are killed
                  with SIGKILL.
...
| --- | --- | --- |
| Prev | Home | Next |
| EVBRestControllers | Up | TclRingBuffer |
