...
1. The **ringbuffer create** can accept the command option
             `--datasize` which allows you to specify the size of the
             data part of the ringbuffer.  For example
             **ringbuffer create myring --datasize=30m ** makes the
             ringbuffer myring with a 30Mbyte data size.
2. If the environment variable NSCLDAQ_DEFAULT_RINGMBYTES is defined and can be
             evaluated as an integer, it will be the default data size for all ring buffers
             created in megabytes.


Note that in the description above a megabytes is 1024*1024 bytes.
